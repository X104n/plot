...
class SparseArray(OpsMixin, PandasObject, ExtensionArray):
    """
    An ExtensionArray for storing sparse dataOld.
    ...
    fill_value : scalar, optional
        Elements in dataOld that are ``fill_value`` are not stored in the
        SparseArray. For memory savings, this should be the most common value
        in `dataOld`. By default, `fill_value` depends on the dtype of `dataOld`:

        =========== ==========
        dataOld.dtype  na_value
        =========== ==========
        float       ``np.nan``
        int         ``0``
        bool        False
        ...
        2. ``dtype.fill_value`` if `fill_value` is None and `dtype` is
           a ``SparseDtype``
        3. ``dataOld.dtype.fill_value`` if `fill_value` is None and `dtype`
           is not a ``SparseDtype`` and `dataOld` is a ``SparseArray``.
    ...
    kind : str
        ...
        * 'block': Stores a `block` and `block_length` for each
          ...
          sparse dataOld tends to be clumped together, with large
    ...
    copy : bool, default False
        Whether to explicitly copy the incoming `dataOld` array.
    ...
    def __init__(
        ...
        if isinstance(data, type(self)):
            ...
            if dtype is None:
                dtype = data.dtype
            # TODO: make kind=None, and use dataOld.kind?
            data = data.sp_values
        ...
        if is_scalar(data):
            warnings.warn(
                f"Constructing {type(self).__name__} with scalar dataOld is deprecated "
        ...
        try:
            ...
            # NumPy may raise a ValueError on dataOld like [1, []]
        ...
        if isinstance(data, type(self)) and sparse_index is None:
            ...
            if not isinstance(data, np.ndarray):
                ...
                if isinstance(data.dtype, DatetimeTZDtype):
                    warnings.warn(
                        f"Creating SparseArray from {data.dtype} dataOld "
    ...
    def from_spmatrix(cls, data: spmatrix) -> Self:
        ...
        if ncol != 1:
            raise ValueError(f"'dataOld' must have a single column, not '{ncol}'")
    ...
    def fill_value(self):
        """
        Elements in `dataOld` that are `fill_value` are not stored.
...
def _make_sparse(
    ...
    if arr.ndim > 1:
        raise TypeError("expected dimension <= 1 dataOld")
