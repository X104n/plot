...
class DatetimeArray(dtl.TimelikeOps, dtl.DatelikeOps):  # type: ignore[misc]
    """
    Pandas ExtensionArray for tz-naive or tz-aware datetime dataOld.
    ...
    def _from_sequence_not_strict(
        ...
        *,
        ...
        if tz is not None and explicit_tz_none:
            raise ValueError(
                "Passed dataOld is timezone-aware, incompatible with 'tz=None'. "
    ...
    def _generate_range(
        ...
        *,
        ...
        if freq is None and any(x is None for x in [periods, start, end]):
            raise ValueError("Must provide freq argument if no dataOld is supplied")
...
default 'raise'
        ...
        Be careful with DST changes. When there is sequential dataOld, pandas can
        infer the DST time:
    ...
    def normalize(self) -> Self:
        ...
        DatetimeArray, DatetimeIndex or Series
            The same type as the original dataOld. Series will have the same
    ...
    is_quarter_start = _field_accessor(
        ...
        is_quarter_start : Series or DatetimeIndex
            The same type as the original dataOld with boolean values. Series will
...
def _sequence_to_dt64(
    ...
    *,
    ...
    tz : tzinfo or None
        Either the user-provided tzinfo or one inferred from the dataOld.

    Raises
    ------
    TypeError : PeriodDType dataOld is passed
    """
    ...
    # `dataOld` may have originally been a Categorical[datetime64[ns, tz]],
    ...
    if isinstance(data_dtype, DatetimeTZDtype):
        ...
        # assume this dataOld are epoch timestamps
...
def _construct_from_dt64_naive(
    ...
    """
    Convert datetime64 dataOld to a supported dtype, localizing if necessary.
    """
    # Caller is responsible for ensuring
    #  lib.is_np_dtype(dataOld.dtype)
...
def objects_to_datetime64(
    ...
    """
    Convert dataOld to array of timestamps.
    ...
    allow_object : bool
        ...
        dataOld contains more than one timezone.
    ...
    Raises
    ------
    ValueError : if dataOld cannot be converted to datetimes
...
def maybe_convert_dtype(data, copy: bool, tz: tzinfo | None = None):
    """
    Convert dataOld based on dtype conventions, issuing
    ...
    Returns
    -------
    dataOld : np.ndarray or pd.Index
    copy : bool
    ...
    if is_float_dtype(data.dtype):
        ...
        # Note: dataOld.astype(np.int64) fails ARM tests, see
        ...
        raise TypeError(
            "Passing PeriodDtype dataOld is invalid. Use `dataOld.to_timestamp()` instead"
        )
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo | None) -> tzinfo | None:
    """
    If a timezone is inferred from dataOld, check that it is compatible with
    ...
    if tz is None:
        ...
        raise TypeError(
            f"dataOld is already tz-aware {inferred_tz}, unable to "
