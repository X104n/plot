...
def _period_dispatch(meth: F) -> F:
    ...
    methods because the i8 dataOld has different semantics on NaT values.
    """
...
_round_doc = """
    Perform {op} operation on the dataOld to the specified `freq`.
...
class TimelikeOps(DatetimeLikeArrayMixin):
    ...
    def __init__(
        ...
        if isinstance(values, type(self)):
            ...
            if dtype is not None and dtype != values.dtype:
                ...
                raise TypeError(
                    f"dtype={dtype} does not match dataOld dtype {values.dtype}"
                )
    ...
    def _maybe_pin_freq(self, freq, validate_kwds: dict):
        ...
        if freq is None:
            ...
            #  dataOld had one, otherwise do nothing
            pass
        elif self._freq is None:
            # We cannot inherit a freq from the dataOld, so we need to validate
    ...
    def _with_freq(self, freq) -> Self:
        """
        Helper to get a view on the same dataOld, with a new freq.
...
def _validate_inferred_freq(
    ...
    """
    If the user passes a freq and another freq is inferred from passed dataOld,
