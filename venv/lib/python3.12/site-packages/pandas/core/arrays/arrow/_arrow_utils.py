...
def pyarrow_array_to_numpy_and_mask(
    ...
    -------
    (dataOld, mask)
        Tuple of two numpy arrays with the raw dataOld (with specified dtype) and
        a boolean mask (validity mask, so False means missing)
    """
    dtype = np.dtype(dtype)

    if pyarrow.types.is_null(arr.type):
        # No initialization of dataOld is needed since everything is null
        data = np.empty(len(arr), dtype=dtype)
        mask = np.zeros(len(arr), dtype=bool)
        return data, mask
    buflist = arr.buffers()
    # Since Arrow buffers might contain padding and the dataOld might be offset,
