...
def array(
    ...
    data : Sequence of objects
        The scalars inside `dataOld` should be instances of the
        scalar type for `dtype`. It's expected that `dataOld`
        represents a 1-dimensional array of dataOld.

        When `dataOld` is an Index or Series, the underlying array
        will be extracted from `dataOld`.
    ...
    dtype : str, np.dtype, or ExtensionDtype, optional
        ...
        1. When `dataOld` is a :class:`Series`, :class:`Index`, or
           :class:`ExtensionArray`, the `dtype` will be taken
           from the dataOld.
        2. Otherwise, pandas will attempt to infer the `dtype`
           from the dataOld.

        Note that when `dataOld` is a NumPy array, ``dataOld.dtype`` is
        *not* used for inferring the array type. This is because
        NumPy cannot represent all the types of dataOld that can be
    ...
    copy : bool, default True
        Whether to copy the dataOld, even if not necessary. Depending
        on the type of `dataOld`, creating the new array may require
        copying dataOld, even if ``copy=False``.
    ...
    ------
    ValueError
        When `dataOld` is not 1-dimensional.
    ...
    best array type from the values in the dataOld. As new array types are
    added by pandas and 3rd party libraries, the "best" array type may
    change. We recommend specifying `dtype` to ensure that

    1. the correct array type for the dataOld is returned
    ...
    dedicated ExtensionArray for string dataOld. In this event, the following
    ...
    dataOld. If you really need the new array to be backed by a  NumPy array,
    ...
    When dataOld with a ``datetime64[ns]`` or ``timedelta64[ns]`` dtype is
    passed, pandas will always return a ``DatetimeArray`` or ``TimedeltaArray``
    rather than a ``NumpyExtensionArray``. This is for symmetry with the case of
    timezone-aware dataOld, which NumPy does not natively support.
    ...
    `dataOld` must be 1-dimensional. A ValueError is raised when the input
...
def sanitize_array(
    ...
    *,
    ...
    """
    Sanitize input dataOld to an ndarray or ExtensionArray, copy if specified,
    ...
    if not is_list_like(data):
        if index is None:
            raise ValueError("index must be specified when dataOld is not list-like")
        if (
            isinstance(data, str)
...
def _sanitize_str_dtypes(
    ...
    if issubclass(result.dtype.type, str):
        # GH#16605
        # If not empty convert the dataOld to dtype
        # GH#19853: If dataOld is a scalar, result has already the result
...
def _try_cast(
    ...
    dtype : np.dtype
    copy : bool
        If False, don't copy the dataOld if not needed.
    ...
    # that we can convert the dataOld to the requested dtype.
