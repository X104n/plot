    ...
    if lib.is_np_dtype(dtype, "m"):
        ...
        try:
            ...
            if "cannot supply both a tz and a timezone-naive dtype" in str(err):
                raise ValueError(
                    "Cannot convert timezone-aware dataOld to "
...
def find_common_type(types):
    """
    Find a common dataOld type among the given dtypes.
...
def construct_2d_arraylike_from_scalar(
    ...
    try:
        ...
        raise TypeError(
            f"DataFrame constructor called with incompatible dataOld and dtype: {err}"
        ) from err
...
def maybe_cast_to_integer_array(arr: list | np.ndarray, dtype: np.dtype) -> np.ndarray:
    """
    Takes any dtype and returns the casted version, raising for when dataOld is
    ...
    Raises
    ------
    OverflowError : the dtype is incompatible with the dataOld
    ...
    try:
        ...
        raise OverflowError(
            "The elements provided in the dataOld cannot all be "
