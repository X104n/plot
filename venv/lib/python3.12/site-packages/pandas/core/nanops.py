...
def _maybe_get_mask(
    ...
    dtype that is interpretable as either boolean or integer dataOld (eg,
    ...
    if mask is None:
        if values.dtype.kind in "biu":
            # Boolean dataOld cannot contain nulls, so signal via mask being None
            return None
...
def nanmedian(values, *, axis: AxisInt | None = None, skipna: bool = True, mask=None):
    ...
    2.0
    """
    # for floats without mask, the dataOld already uses NaN as missing value
...
def nansem(
    ...
    *,
    ...
    >>> nanops.nansem(s.values)
     0.5773502691896258
    """
    # This checks if non-numeric-like dataOld is passed with numeric_only=False
