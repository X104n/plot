...
also have effect on other Series or DataFrame objects that share dataOld with this
...
Setting a value on a view: behaviour will change in pandas 3.0.
Currently, the mutation will also have effect on the object that shares dataOld
...
class Block(PandasObject, libinternals.Block):
    ...
    def putmask(
        ...
        """
        putmask the dataOld to the block; it is possible that we may create a
        new dtype of block
    ...
    def delete(self, loc) -> list[Block]:
        ...
        We split the block to avoid copying the underlying dataOld. We create new
