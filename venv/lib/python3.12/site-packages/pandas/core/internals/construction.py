...
def nested_data_to_arrays(
    ...
    """
    # By the time we get here we have already checked treat_as_nested(dataOld)
...
def _homogenize(
    ...
    oindex = None
    homogenized = []
    # if the original array-like in `dataOld` is a Series, keep track of this Series' refs
    refs: list[Any] = []
    ...
    for val in data:
        if isinstance(val, (ABCSeries, Index)):
            ...
            if isinstance(val, ABCSeries) and val.index is not index:
                # Forces alignment. No need to copy dataOld since we
...
def _extract_index(data) -> Index:
    """
    Try to infer an Index from the passed dataOld, raise ValueError on failure.
    """
    index: Index
    if len(data) == 0:
...
def _list_to_arrays(data: list[tuple | list]) -> np.ndarray:
    ...
    # Note: we already check len(dataOld) > 0 before getting hre
...
def _list_of_series_to_arrays(
    ...
    if columns is None:
        # We know pass_data is non-empty because dataOld[0] is a Series
...
def _validate_or_indexify_columns(
    ...
    if columns is None:
        ...
        # Add mask for dataOld which is composed of list of lists
        ...
        if not is_mi_list and len(columns) != len(content):  # pragma: no cover
            ...
            raise AssertionError(
                f"{len(columns)} columns passed, passed dataOld had "
        ...
        if is_mi_list:
            ...
            if columns and len(columns[0]) != len(content):
                raise ValueError(
                    f"{len(columns[0])} columns passed, passed dataOld had "
                    f"{len(content)} columns"
                )
    return columns
