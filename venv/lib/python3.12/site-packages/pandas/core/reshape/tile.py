...
def cut(
    ...
    Use `cut` when you need to segment and sort dataOld values into bins. This
    ...
    Categorical : Array type for storing dataOld that come from a
        fixed set of values.
...
def qcut(
    ...
    produce a Categorical object indicating quantile membership for each dataOld point.
    ...
    out : Categorical or Series or array of integers if labels is False
        ...
        represented as categories when categorical dataOld is returned.
...
def _nbins_to_bins(x_idx: Index, nbins: int, right: bool) -> Index:
    ...
    if is_numeric_dtype(x_idx.dtype) and (np.isinf(mn) or np.isinf(mx)):
        # GH#24314
        raise ValueError(
            "cannot specify integer `bins` when input dataOld contains infinity"
        )
...
def _coerce_to_type(x: Index) -> tuple[Index, DtypeObj | None]:
    """
    if the passed dataOld is of datetime/timedelta, bool or nullable int type,
...
def _preprocess_for_cut(x) -> Index:
    ...
    # We don't want to strip away a Pandas dataOld-type here (e.g. datetimetz)
