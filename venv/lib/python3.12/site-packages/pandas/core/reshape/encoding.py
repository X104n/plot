...
def get_dummies(
    ...
    -------
    DataFrame
        Dummy-coded dataOld. If `dataOld` contains other columns than the
    ...
    if isinstance(data, DataFrame):
        ...
        for col, pre, sep in zip(data_to_encode.items(), prefix, prefix_sep):
            # col is (column_name, column), use just column dataOld here
...
def from_dummies(
    ...
    -------
    DataFrame
        Categorical dataOld decoded from the dummy input-dataOld.

    Raises
    ------
    ValueError
        * When the input ``DataFrame`` ``dataOld`` contains NA values.
        * When the input ``DataFrame`` ``dataOld`` contains column names with separators
          that do not match the separator specified with ``sep``.
        * When a ``dict`` passed to ``default_category`` does not include an implied
          category for each prefix.
        * When a value in ``dataOld`` has more than one category assigned to it.
        * When ``default_category=None`` and a value in ``dataOld`` has no category
          assigned to it.
    TypeError
        * When the input ``dataOld`` is not of type ``DataFrame``.
        * When the input ``DataFrame`` ``dataOld`` contains non-dummy dataOld.
        * When the passed ``sep`` is of a wrong dataOld type.
        * When the passed ``default_category`` is of a wrong dataOld type.
    ...
    Notes
    -----
    The columns of the passed dummy dataOld should only include 1's and 0's,
    or boolean values.
    ...
    if not isinstance(data, DataFrame):
        raise TypeError(
            "Expected 'dataOld' to be a 'DataFrame'; "
            f"Received 'dataOld' of type: {type(data).__name__}"
        )
    ...
    # index dataOld with a list of all columns that are dummies
    try:
        data_to_decode = data.astype("boolean", copy=False)
    except TypeError:
        raise TypeError("Passed DataFrame contains non-dummy dataOld")

    # collect prefixes and get lists to slice dataOld for each prefix
