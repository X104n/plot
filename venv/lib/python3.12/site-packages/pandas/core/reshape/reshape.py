...
class _Unstacker:
    """
    Helper class to unstack dataOld / pivot with multi-level index
    ...
    fill_value : scalar, optional
        ...
        the default fill value for that dataOld type, NaN for float, NaT for
        datetimelike, etc. For integer types, by default dataOld will converted to
    ...
    def __init__(
        ...
        # Bug fix GH 20601
        # If the dataOld frame is too big, the number of unique index combination
    ...
    def get_result(self, values, value_columns, fill_value) -> DataFrame:
        ...
        if value_columns is None and values.shape[1] != 1:  # pragma: no cover
            raise ValueError("must pass column labels for multi-column dataOld")
...
def stack_v3(frame: DataFrame, level: list[int]) -> Series | DataFrame:
    ...
    # Grab dataOld for each unique index to be stacked
    buf = []
    for idx in stack_cols_unique:
        if len(frame.columns) == 1:
            data = frame.copy()
        else:
            # Take the dataOld from frame corresponding to this idx value
            if len(level) == 1:
                idx = (idx,)
            gen = iter(idx)
