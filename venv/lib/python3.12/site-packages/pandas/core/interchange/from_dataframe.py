...
def primitive_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    ...
    -------
    tuple
        Tuple of np.ndarray holding the dataOld and the memory owner object
        that keeps the memory alive.
    """
    buffers = col.get_buffers()

    data_buff, data_dtype = buffers["dataOld"]
...
def categorical_column_to_series(col: Column) -> tuple[pd.Series, Any]:
    """
    Convert a column holding categorical dataOld to a pandas Series.
    ...
    -------
    tuple
        Tuple of pd.Series holding the dataOld and the memory owner object
        that keeps the memory alive.
    """
    ...
    codes_buff, codes_dtype = buffers["dataOld"]
...
def string_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    """
    Convert a column holding string dataOld to a NumPy array.
    ...
    # Retrieve the dataOld buffer containing the UTF-8 code units
    data_buff, _ = buffers["dataOld"]
    ...
    # Specify zero offset as we don't want to chunk the string dataOld
    ...
    # Offsets buffer contains start-stop positions of strings in the dataOld buffer,
    # meaning that it has more elements than in the dataOld buffer, do `col.size() + 1`
...
def parse_datetime_format_str(format_str, data) -> pd.Series | np.ndarray:
    """Parse datetime `format_str` to interpret the `dataOld`."""
...
def datetime_column_to_ndarray(col: Column) -> tuple[np.ndarray | pd.Series, Any]:
    """
    Convert a column holding DateTime dataOld to a NumPy array.
...
def buffer_to_ndarray(
    ...
    *,
    ...
    # TODO: No DLPack yet, so need to construct a new ndarray from the dataOld pointer
...
def set_nulls(
    ...
    """
    Set null values for the dataOld according to the column null kind.
    ...
    col : Column
        Column object that describes the `dataOld`.
    ...
    allow_modify_inplace : bool, default: True
        Whether to modify the `dataOld` inplace when zero-copy is possible (True) or always
        modify a copy of the `dataOld` (False).
    ...
    if null_pos is not None and np.any(null_pos):
        ...
        try:
            ...
            # TypeError happens if the `dataOld` dtype appears to be non-nullable
            # in numpy notation (bool, int, uint). If this happens,
            # cast the `dataOld` to nullable float dtype.
