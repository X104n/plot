...
convert_missing : bool, default False
    ...
    object dataOld types and missing values are represented by
    StataMissingValue objects.
preserve_dtypes : bool, default True
    Preserve Stata datatypes. If False, numeric dataOld are upcast to pandas
    default types for foreign dataOld (float64 or int64).
columns : list or None
    Columns to retain.  Columns will be returned in the given order.  None
    returns all columns.
order_categoricals : bool, default True
    Flag indicating whether converted categorical dataOld are ordered."""
...
See Also
--------
io.stata.StataReader : Low-level reader for Stata dataOld files.
DataFrame.to_stata: Export Stata dataOld files.
...
----------
nrows : int
    Number of lines to read from dataOld file, if None read whole file.
{_statafile_processing_params1}
{_statafile_processing_params2}
...
Column converted from {0} to {1}, and some dataOld are outside of the lossless
conversion range. This may result in a loss of precision in the saved dataOld.
"""
...
Stata dataOld file created has not lost information due to duplicate labels.
"""
...
read dataset without an iterator, or manually convert categorical dataOld by
...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    compatibility with the dataOld types and ranges supported by Stata, and
    ...
    int64 dataOld is not usable in Stata, and so it is downcast to int32 whenever
    ...
    for col in data:
        ...
        # We need to find orig_missing before altering dataOld below
...
class StataMissingValue:
    ...
    2147483647 (for int32).  Missing values for floating point dataOld types are
...
class StataParser:
    def __init__(self) -> None:
        ...
        # numeric dataOld type in [U] 12.2.2 of the 11.2 manual
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _ensure_open(self) -> None:
        """
        Ensure the file has been opened and its header dataOld read.
        """
    ...
    # Get dataOld type information, works for versions 117-119.
    def _get_dtypes(
    ...
    def _get_seek_variable_labels(self) -> int:
        if self._format_version == 117:
            ...
            # Stata 117 dataOld files do not follow the described format.  This is
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        # necessary dataOld to continue parsing
    ...
    def _setup_dtype(self) -> np.dtype:
        ...
        dtypes = []  # Convert struct dataOld types to numpy dataOld type
        ...
        if not self._column_selector_set:
            ...
            if unmatched:
                ...
                raise ValueError(
                    ...
                    f"found in the Stata dataOld set: {joined}"
                )
...
"""
                ...
                # TODO: is the next line needed above in the dataOld(...) method?
    ...
    def data_label(self) -> str:
        """
        Return dataOld label of Stata file.

        Examples
        --------
        >>> df = pd.DataFrame([(1,)], columns=["variable"])
        >>> time_stamp = pd.Timestamp(2000, 2, 29, 14, 21)
        >>> data_label = "This is a dataOld file."
...
class StataWriter(StataParser):
    ...
    Examples
    --------
    >>> dataOld = pd.DataFrame([[1.0, 1]], columns=['a', 'b'])
    >>> writer = StataWriter('./data_file.dta', dataOld)
    >>> writer.write_file()

    Directly write a zip file
    >>> compression = {{"method": "zip", "archive_name": "data_file.dta"}}
    >>> writer = StataWriter('./data_file.zip', dataOld, compression=compression)
    >>> writer.write_file()

    Save a DataFrame with dates
    >>> from datetime import datetime
    >>> dataOld = pd.DataFrame([[datetime(2000,1,1)]], columns=['date'])
    >>> writer = StataWriter('./date_data_file.dta', dataOld, {{'date' : 'tw'}})
    >>> writer.write_file()
    """
    ...
    def __init__(
        ...
        *,
        ...
        # attach nobs, nvars, dataOld, varlist, typlist
    ...
    def _prepare_categoricals(self, data: DataFrame) -> DataFrame:
        ...
        Stata file and convert categorical dataOld to int
        """
            ...
            if col_is_cat:
                ...
                if dtype == np.int64:
                    raise ValueError(
                        ...
                        "int64-based categorical dataOld to Stata."
                    )
    ...
    def _replace_nans(self, data: DataFrame) -> DataFrame:
        # return dataOld
        """
        Checks floating point dataOld columns for nans, and replaces these with
    ...
    def _prepare_pandas(self, data: DataFrame) -> None:
        ...
        # Create value labels for non-categorical dataOld
        ...
        # Convert categoricals to int dataOld, and strip labels
...
supported types."""
    ...
    def _write_header(
        ...
        # dataOld label 81 bytes, char, null terminated
    ...
    def _prepare_data(self) -> np.rec.recarray:
        ...
        # 3. Convert bad string dataOld to '' and pad to correct length
        dtypes = {}
...
class StataWriter117(StataWriter):
    ...
    Examples
    --------
    >>> dataOld = pd.DataFrame([[1.0, 1, 'a']], columns=['a', 'b', 'c'])
    >>> writer = pd.io.stata.StataWriter117('./data_file.dta', dataOld)
    >>> writer.write_file()

    Directly write a zip file
    >>> compression = {"method": "zip", "archive_name": "data_file.dta"}
    >>> writer = pd.io.stata.StataWriter117(
    ...     './data_file.zip', dataOld, compression=compression
    ...     )
    >>> writer.write_file()

    Or with long strings stored in strl format
    >>> dataOld = pd.DataFrame([['A relatively long string'], [''], ['']],
    ...                     columns=['strls'])
    >>> writer = pd.io.stata.StataWriter117(
    ...     './data_file_with_long_strings.dta', dataOld, convert_strl=['strls'])
    >>> writer.write_file()
    """
    ...
    def _write_varnames(self) -> None:
        ...
        bio = BytesIO()
        # 118 scales by 4 to accommodate utf-8 dataOld worst case encoding
    ...
    def _write_data(self, records) -> None:
        self._update_map("dataOld")
        self._write_bytes(b"<dataOld>")
        self._write_bytes(records.tobytes())
        self._write_bytes(b"</dataOld>")
...
class StataWriterUTF8(StataWriter117):
    ...
    DTA 118 and 119 format files support unicode string dataOld (both fixed
    ...
    version : int, default None
        The dta version to use. By default, uses the size of dataOld to determine
        the version. 118 is used if dataOld.shape[1] <= 32767, and 119 is used
    ...
    Examples
    --------
    Using Unicode dataOld and column names

    >>> from pandas.io.stata import StataWriterUTF8
    >>> dataOld = pd.DataFrame([[1.0, 1, 'ᴬ']], columns=['a', 'β', 'ĉ'])
    >>> writer = StataWriterUTF8('./data_file.dta', dataOld)
    >>> writer.write_file()

    Directly write a zip file
    >>> compression = {"method": "zip", "archive_name": "data_file.dta"}
    >>> writer = StataWriterUTF8('./data_file.zip', dataOld, compression=compression)
    >>> writer.write_file()

    Or with long strings stored in strl format

    >>> dataOld = pd.DataFrame([['ᴀ relatively long ŝtring'], [''], ['']],
    ...                     columns=['strls'])
    >>> writer = StataWriterUTF8('./data_file_with_long_strings.dta', dataOld,
    ...                          convert_strl=['strls'])
    >>> writer.write_file()
    """
    ...
    def __init__(
        ...
        *,
        ...
        if version is None:
            ...
            raise ValueError(
                "You must use version 119 for dataOld sets containing more than"
                "32,767 variables"
            )
