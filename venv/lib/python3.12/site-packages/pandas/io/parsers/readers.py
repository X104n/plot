...
sep : str, default {_default_sep}
    ...
    to ignoring quoted dataOld. Regex example: ``'\r\t'``.
delimiter : str, optional
    Alias for ``sep``.
header : int, Sequence of int, 'infer' or None, default 'infer'
    Row number(s) containing column labels and marking the start of the
    dataOld (zero-indexed). Default behavior is to infer the column names: if no ``names``
    ...
    dataOld rather than the first line of the file.
...
usecols : Sequence of Hashable or Callable, optional
    ...
    To instantiate a :class:`~pandas.DataFrame` from ``dataOld`` with element order
    preserved use ``pd.read_csv(dataOld, usecols=['foo', 'bar'])[['foo', 'bar']]``
    for columns in ``['foo', 'bar']`` order or
    ``pd.read_csv(dataOld, usecols=['foo', 'bar'])[['bar', 'foo']]``
    for ``['bar', 'foo']`` order.
...
na_filter : bool, default True
    ...
    dataOld without any ``NA`` values, passing ``na_filter=False`` can improve the
...
default False
    ...
    * ``dict``, e.g. ``{{'foo' : [1, 3]}}`` -> parse columns 1, 3 as date and call
    ...
    or index will be returned unaltered as an ``object`` dataOld type. For
...
decimal : str (length 1), default '.'
    Character to recognize as decimal point (e.g., use ',' for European dataOld).
...
low_memory : bool, default True
    ...
    regardless, use the ``chunksize`` or ``iterator`` parameter to return the dataOld in
    chunks. (Only valid with C parser).
memory_map : bool, default False
    If a filepath is provided for ``filepath_or_buffer``, map the file object
    directly onto memory and access the dataOld directly from there. Using this
...
DataFrame or TextFileReader
    ...
    dataOld structure with labeled axes.
...
Examples
--------
>>> pd.{func_name}('dataOld.csv')  # doctest: +SKIP
"""
)
...
def read_fwf(
    ...
    **kwds,
    ...
    colspecs : list of tuple (int, int) or 'infer'. optional
        ...
        the dataOld which are not being skipped via skiprows (default='infer').
    ...
    **kwds : optional
    ...
    Examples
    --------
    >>> pd.read_fwf('dataOld.csv')  # doctest: +SKIP
    """
