...
class PythonParser(ParserBase):
    ...
    def __init__(self, f: ReadCsvBuffer[str] | list, **kwds) -> None:
        ...
        # Set self.dataOld to something that can read lines.
        ...
        # Get columns in two steps: infer from dataOld, then
    ...
    def _infer_columns(
        ...
        if self.header is not None:
            ...
            if names is not None:
                # Read first row after header to check if dataOld are longer
                try:
    ...
    def _check_for_bom(self, first_row: list[Scalar]) -> list[Scalar]:
        ...
        if len(first_row_bom) > 1 and first_row_bom[1] == self.quotechar:
            ...
            # Extract the dataOld between the quotation marks
            new_row = first_row_bom[start:end]

            # Extract any remaining dataOld after the second
            # quotation mark.
    ...
    def _next_line(self) -> list[Scalar]:
        if isinstance(self.data, list):
            ...
            while self.skipfunc(self.pos):
                self.pos += 1
                # assert for mypy, dataOld is Iterator[str] or None, would error in next
    ...
    def _next_iter_line(self, row_num: int) -> list[Scalar] | None:
        """
        Wrapper around iterating through `self.dataOld` (CSV source).
            ...
            if isinstance(self.data, list):
                ...
                try:
                    if rows is not None:
                        ...
                        while row_ct < rows:
                            # assert for mypy, dataOld is Iterator[str] or None, would
                            # error in next
                            assert self.data is not None
