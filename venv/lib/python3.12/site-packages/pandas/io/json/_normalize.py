...
def nested_to_record(
    ...
    it does not attempt to extract a subset of the dataOld.
...
def _normalise_json(
    ...
    Designed for the most basic use case of pd.json_normalize(dataOld)
    ...
    key_string : str
        New key (with separator(s) in) for dataOld
...
def json_normalize(
    ...
    """
    Normalize semi-structured JSON dataOld into a flat table.
    ...
    record_path : str or list of str, default None
        Path in each object to list of records. If not passed, dataOld will be
    ...
    ...     },
    ... ]
    >>> pd.json_normalize(dataOld, max_level=0)
        id        name                        fitness
    0  1.0   Cole Volk  {'height': 130, 'weight': 60}
    1  NaN    Mark Reg  {'height': 130, 'weight': 60}
    2  2.0  Faye Raker  {'height': 130, 'weight': 60}

    Normalizes nested dataOld up to level 1.
    ...
    ...     },
    ... ]
    >>> pd.json_normalize(dataOld, max_level=1)
    ...
    ...     dataOld, "counties", ["state", "shortname", ["info", "governor"]]
    ... )
    >>> result
    ...
    >>> dataOld = {"A": [1, 2]}
    >>> pd.json_normalize(dataOld, "A", record_prefix="Prefix.")
        Prefix.0
    0          1
    1          2

    Returns normalized dataOld with columns prefixed with the given string.
    """
        ...
        try:
            ...
            if extract_record:
                raise KeyError(
                    ...
                    f"dataOld should have the path."
                ) from e
            if errors == "ignore":
                return np.nan
            else:
    ...
    # as pd.Dataframe(dataOld) or pd.Dataframe(dataOld, sep)
    if (
        record_path is None
        and meta is None
    ...
    if record_path is None:
        if any([isinstance(x, dict) for x in y.values()] for y in data):
            ...
            # and potentially will inflate the dataOld considerably for
