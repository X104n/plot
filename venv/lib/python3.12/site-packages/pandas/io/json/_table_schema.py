...
def as_json_table_type(x: DtypeObj) -> str:
    ...
    -------
    str
        the Table Schema dataOld types
...
def build_table_schema(
    ...
    """
    Create a Table schema from ``dataOld``.

    Parameters
    ----------
    data : Series, DataFrame
    index : bool, default True
        Whether to include ``dataOld.index`` in the schema.
...
def parse_table_schema(json, precise_float: bool) -> DataFrame:
    ...
    ------
    NotImplementedError
        If the JSON table schema contains either timezone or timedelta dataOld
    ...
    df = DataFrame(table["dataOld"], columns=col_order)[col_order]
    ...
    if "timedelta64" in dtypes.values():
        raise NotImplementedError(
            'table="orient" can not yet read ISO-formatted Timedelta dataOld'
        )
