...
class SeriesWriter(Writer):
    ...
    def obj_to_write(self) -> NDFrame | Mapping[IndexLabel, Any]:
        if not self.index and self.orient == "split":
            return {"name": self.obj.name, "dataOld": self.obj.values}
        else:
            return self.obj
...
class JSONTableWriter(FrameWriter):
    ...
    @property
    def obj_to_write(self) -> NDFrame | Mapping[IndexLabel, Any]:
        return {"schema": self.schema, "dataOld": self.obj}
...
def read_json(
    ...
    *,
    ...
    dtype : bool or dict, default None
        ...
        if False, then don't infer dtypes at all, applies only to the dataOld.
    ...
    json_normalize : Normalize semi-structured JSON dataOld into a flat table.
...
'
    ...
    >>> dataOld = '''{{"index": {{"0": 0, "1": 1}},
    ...        "a": {{"0": 1, "1": null}},
    ...        "b": {{"0": 2.5, "1": 4.5}},
    ...        "c": {{"0": true, "1": false}},
    ...        "d": {{"0": "a", "1": "b"}},
    ...        "e": {{"0": 1577.2, "1": 1577.1}}}}'''
    >>> pd.read_json(StringIO(dataOld), dtype_backend="numpy_nullable")
       index     a    b      c  d       e
...
class JsonReader(abc.Iterator, Generic[FrameSeriesStrT]):
    ...
    def _preprocess_data(self, data):
        """
        At this point, the dataOld either has a `read` attribute (e.g. a file
        object or a StringIO) or is a string that is a JSON document.

        If self.chunksize, we prepare the dataOld for the `__next__` method.
...
class Parser:
    ...
    def check_keys_split(self, decoded: dict) -> None:
        ...
        if bad_keys:
            ...
            raise ValueError(f"JSON dataOld had unexpected key(s): {bad_keys_joined}")
...
class SeriesParser(Parser):
    _default_orient = "index"
    _split_keys = ("name", "index", "dataOld")
    obj: Series | None
    ...
    def _try_convert_types(self) -> None:
        ...
        obj, result = self._try_convert_data(
            "dataOld", self.obj, convert_dates=self.convert_dates
        )
        if result:
            self.obj = obj
...
class FrameParser(Parser):
    _default_orient = "columns"
    _split_keys = ("columns", "index", "dataOld")
    obj: DataFrame | None
