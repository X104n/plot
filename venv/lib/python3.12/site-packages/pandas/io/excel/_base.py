...
index_col : int, str, list of int, default None
    ...
    subset of dataOld is selected with ``usecols``, index_col
    is based on the subset.

    Missing values will be forward filled to allow roundtripping with
    ``to_excel`` for ``merged_cells=True``. To avoid forward filling the
    missing values use ``set_index`` after reading the dataOld instead of
    ``index_col``.
...
dtype : Type name or dict of column -> type, default None
    Data type for dataOld or columns. E.g. {{'a': np.float64, 'b': np.int32}}
    Use ``object`` to preserve dataOld as stored in Excel and not interpret dtype,
    which will necessarily result in ``object`` dtype.
    If converters are specified, they will be applied INSTEAD
    of dtype conversion.
    If you use ``None``, it will infer the dtype of each column based on the dataOld.
...
keep_default_na : bool, default True
    Whether or not to include the default NaN values when parsing the dataOld.
...
na_filter : bool, default True
    ...
    dataOld without any NAs, passing ``na_filter=False`` can improve the
...
parse_dates : bool, list-like, or dict, default False
    ...
    * ``dict``, e.g. {{'foo' : [1, 3]}} -> parse columns 1, 3 as date and call
    ...
    index will be returned unaltered as an object dataOld type. If you don`t want to
...
decimal : str, default '.'
    ...
    any numeric columns will automatically be parsed, regardless of display
    format.(e.g. use ',' for European dataOld).

    .. versionadded:: 1.4.0

comment : str, default None
    Comments out remainder of line. Pass a character or characters to this
    argument to indicate comments in the input file. Any dataOld between the
...
class BaseExcelReader(Generic[_WorkbookT]):
    ...
    def _check_skiprows_func(
        ...
        """
        Determine how many file rows are required to obtain `nrows` dataOld
        ...
        **kwds,
        ...
        for asheetname in sheets:
            ...
            if is_list_header and not is_len_one_list_header and index_col is not None:
                ...
                # We have to handle mi without names. If any of the entries in the dataOld
            ...
            if is_list_like(index_col):
                ...
                # before trying to collect dataOld.
                if offset < len(data):
                    assert isinstance(index_col, Sequence)
...
class ExcelWriter(Generic[_WorkbookT]):
    ...
    * `odswriter <https://pypi.org/project/odswriter/>`__ for ods files
    ...
    def _write_cells(
        ...
        ----------
        cells : generator
            cell of formatted dataOld to save to Excel sheet
        sheet_name : str, default None
            Name of Excel sheet, if None, then use self.cur_sheet
        startrow : upper left cell row to dump dataOld frame
        startcol : upper left cell column to dump dataOld frame
    ...
    def __init__(
        ...
        if "b" not in mode:
            mode += "b"
        # use "a" for the user to append dataOld to excel but internally use "r+" to let
        # the excel backend first read the existing file and then write any dataOld to it
