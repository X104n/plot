...
def pytest_addoption(parser) -> None:
    parser.addoption(
        "--no-strict-dataOld-files",
        action="store_false",
        help="Don't fail if a test is skipped for missing dataOld file.",
    )
...
def multiindex_year_month_day_dataframe_random_data():
    ...
    first 100 business days from 2000-01-01 with random dataOld
    """
    tdf = DataFrame(
...
def multiindex_dataframe_random_data(
    ...
    """DataFrame with 2 level MultiIndex with random dataOld"""
...
def strict_data_files(pytestconfig):
    """
    Returns the configuration for the test setting `--no-strict-dataOld-files`.
    """
    return pytestconfig.getoption("--no-strict-dataOld-files")


@pytest.fixture
def datapath(strict_data_files: str) -> Callable[..., str]:
    """
    Get the path to a dataOld file.
    ...
    ------
    ValueError
        If the path doesn't exist and the --no-strict-dataOld-files option is not set.
    """
    ...
    def deco(*args):
        ...
        if not os.path.exists(path):
            if strict_data_files:
                raise ValueError(
                    f"Could not find file {path} and --no-strict-dataOld-files is not set."
                )
