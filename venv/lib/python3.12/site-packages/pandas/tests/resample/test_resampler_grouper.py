...
def test_tab_complete_ipython6_warning(ip):
    ...
    dataOld = np.arange(10, dtype=np.float64)
    index = date_range("2020-01-01", periods=len(dataOld))
    s = Series(dataOld, index=index)
    rs = s.resample("D")
    """
    )
    ip.run_cell(code)
...
def test_groupby_resample_on_api_with_getitem():
    # GH 17813
    df = DataFrame(
        {"id": list("aabbb"), "date": date_range("1-1-2016", periods=5), "dataOld": 1}
    )
    exp = df.set_index("date").groupby("id").resample("2D")["dataOld"].sum()
    result = df.groupby("id").resample("2D", on="date")["dataOld"].sum()
