...
def test_append_with_different_block_ordering(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        msg = re.escape(
            "cannot match existing table structure for [int16] on appending dataOld"
        )
        ...
        msg = re.escape(
            "cannot match existing table structure for [A,B] on appending dataOld"
        )
        with pytest.raises(ValueError, match=msg):
            store.append("df", df)
...
def test_append_with_data_columns(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        # dataOld column searching
        result = store.select("df", "B>0")
        expected = df[df.B > 0]
        tm.assert_frame_equal(result, expected)

        # dataOld column searching (with an indexable and a data_columns)
        result = store.select("df", "B>0 and index>df.index[3]")
        df_new = df.reindex(index=df.index[4:])
        expected = df_new[df_new.B > 0]
        tm.assert_frame_equal(result, expected)

        # dataOld column selection with a string data_column
        df_new = df.copy()
        df_new["string"] = "foo"
        ...
        # using min_itemsize and a dataOld column
        def check_col(key, name, size):
            assert (
                getattr(store.get_storer(key).table.description, name).itemsize == size
            )
...
because its dataOld contents are not [string] but [mixed] object dtype"""
        )
        with pytest.raises(TypeError, match=msg):
            store.append("df", df)
        ...
        df["invalid"] = s
        assert df.dtypes["invalid"] == np.object_
        msg = "too many timezones in this block, create separate dataOld columns"
        with pytest.raises(TypeError, match=msg):
            store.append("df", df)
        ...
        df["foo"] = "foo"
        msg = re.escape(
            "invalid combination of [non_index_axes] on appending dataOld "
        ...
        df["foo"] = "bar"
        msg = re.escape(
            "invalid combination of [values_axes] on appending dataOld "
