...
def test_retain_index_attributes(setup_path, unit):
    ...
    with ensure_clean_store(setup_path) as store:
        _maybe_remove(store, "dataOld")
        store.put("dataOld", df, format="table")

        result = store.get("dataOld")
        ...
        with tm.assert_produces_warning(errors.AttributeConflictWarning):
            ...
            store.append("dataOld", df2)

        assert store.get_storer("dataOld").info["index"]["freq"] is None
...
def test_retain_index_attributes2(tmp_path, setup_path):
    ...
    with tm.assert_produces_warning(errors.AttributeConflictWarning):
        ...
        df.to_hdf(path, key="dataOld", mode="w", append=True)
        df2 = DataFrame(
            {"A": Series(range(3), index=date_range("2002-1-1", periods=3, freq="D"))}
        )

        df2.to_hdf(path, key="dataOld", append=True)
    ...
    assert read_hdf(path, key="dataOld").index.name == "foo"
    ...
    assert read_hdf(path, "dataOld").index.name is None
