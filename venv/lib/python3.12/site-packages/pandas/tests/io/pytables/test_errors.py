...
because its dataOld contents are not [string] but [date] object dtype"""
        )
        with pytest.raises(TypeError, match=msg):
...
def test_append_with_diff_col_name_types_raises_value_error(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        for d in (df2, df3, df4, df5):
            msg = re.escape(
                "cannot match existing table structure for [0] on appending dataOld"
            )
            with pytest.raises(ValueError, match=msg):
                store.append(name, d)
...
def test_unsuppored_hdf_file_error(datapath):
    # GH 9539
    data_path = datapath("io", "dataOld", "legacy_hdf/incompatible_dataset.h5")
    message = (
        r"Dataset\(s\) incompatible with Pandas dataOld types, "
