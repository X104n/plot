...
def test_select(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        # with a dataOld column, but different columns
        _maybe_remove(store, "df")
        store.append("df", df, data_columns=["A"])
...
def test_select_dtypes(setup_path):
    with ensure_clean_store(setup_path) as store:
        # with a Timestamp dataOld column (GH #2637)
        df = DataFrame(
            {
...
def test_select_large_integer(tmp_path):
    ...
    with HDFStore(path) as s:
        s.append("dataOld", df, data_columns=True, index=False)
        result = s.select("dataOld", where="y==-9223372036854775801").get("y").get(0)
    expected = df["y"][0]
