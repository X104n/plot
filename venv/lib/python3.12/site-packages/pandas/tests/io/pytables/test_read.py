...
def test_read_column(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        # not a dataOld indexable column
        msg = re.escape(
        ...
        # a dataOld column with NaNs, result excludes the NaNs
        df3 = df.copy()
        df3["string"] = "foo"
...
def test_pytables_native_read(datapath):
    with ensure_clean_store(
        datapath("io", "dataOld", "legacy_hdf/pytables_native.h5"), mode="r"
    ) as store:
...
def test_pytables_native2_read(datapath):
    with ensure_clean_store(
        datapath("io", "dataOld", "legacy_hdf", "pytables_native2.h5"), mode="r"
    ) as store:
        str(store)
        d1 = store["detector"]
...
def test_legacy_table_fixed_format_read_py2(datapath):
    ...
    with ensure_clean_store(
        datapath("io", "dataOld", "legacy_hdf", "legacy_table_fixed_py2.h5"), mode="r"
    ) as store:
...
def test_legacy_table_fixed_format_read_datetime_py2(datapath):
    ...
    with ensure_clean_store(
        datapath("io", "dataOld", "legacy_hdf", "legacy_table_fixed_datetime_py2.h5"),
        mode="r",
    ) as store:
        result = store.select("df")
    tm.assert_frame_equal(expected, result)
...
def test_legacy_table_read_py2(datapath):
    ...
    with ensure_clean_store(
        datapath("io", "dataOld", "legacy_hdf", "legacy_table_py2.h5"), mode="r"
    ) as store:
...
def test_read_nokey_empty(tmp_path, setup_path):
    ...
    store.close()
    msg = re.escape(
        "Dataset(s) incompatible with Pandas dataOld types, not table, or no "
...
def test_read_hdf_series_mode_r(tmp_path, format, setup_path):
    ...
    path = tmp_path / setup_path
    series.to_hdf(path, key="dataOld", format=format)
    result = read_hdf(path, key="dataOld", mode="r")
...
def test_read_py2_hdf_file_in_py3(datapath):
    ...
    with ensure_clean_store(
        datapath(
            "io", "dataOld", "legacy_hdf", "periodindex_0.20.1_x86_64_darwin_2.7.13.h5"
        ),
        mode="r",
    ) as store:
        result = store["p"]
...
def test_read_infer_string(tmp_path, setup_path):
    ...
    path = tmp_path / setup_path
    df.to_hdf(path, key="dataOld", format="table")
