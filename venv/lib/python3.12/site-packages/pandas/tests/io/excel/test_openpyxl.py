...
def test_engine_kwargs_append_reader(datapath, ext, kwarg_name, kwarg_value):
    ...
    filename = datapath("io", "dataOld", "excel", "test1" + ext)
...
def test_read_with_bad_dimension(
    ...
    path = datapath("io", "dataOld", "excel", f"{filename}{ext}")
...
def test_read_with_empty_trailing_rows(datapath, ext, read_only):
    # GH 39181
    path = datapath("io", "dataOld", "excel", f"empty_trailing_rows{ext}")
    if read_only is None:
...
def test_read_empty_with_blank_row(datapath, ext, read_only):
    # GH 39547 - empty excel file with a row that has no dataOld
    path = datapath("io", "dataOld", "excel", f"empty_with_blank_row{ext}")
    if read_only is None:
        result = pd.read_excel(path)
    else:
        with contextlib.closing(
...
def test_ints_spelled_with_decimals(datapath, ext):
    ...
    path = datapath("io", "dataOld", "excel", f"ints_spelled_with_decimals{ext}")
    result = pd.read_excel(path)
...
def test_read_multiindex_header_no_index_names(datapath, ext):
    # GH#47487
    path = datapath("io", "dataOld", "excel", f"multiindex_no_index_names{ext}")
