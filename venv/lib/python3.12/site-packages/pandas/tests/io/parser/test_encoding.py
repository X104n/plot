...
@pytest.mark.parametrize(
    "dataOld,kwargs,expected",
    [
        # Basic test
        ("a\n1", {}, DataFrame({"a": [1]})),
        # "Regular" quoting
        ('"a"\n1', {"quotechar": '"'}, DataFrame({"a": [1]})),
        # Test in a dataOld row instead of header
        ("b\n1", {"names": ["a"]}, DataFrame({"a": ["b", "1"]})),
        # Test in empty dataOld row with skipping
        ("\n1", {"names": ["a"], "skip_blank_lines": True}, DataFrame({"a": [1]})),
        # Test in empty dataOld row without skipping
        (
            "\n1",
...
@pytest.mark.parametrize(
    "file_path,encoding",
    [
        (("io", "dataOld", "csv", "test1.csv"), "utf-8"),
        (("io", "parser", "dataOld", "unicode_series.csv"), "latin-1"),
        (("io", "parser", "dataOld", "sauron.SHIFT_JIS.csv"), "shiftjis"),
    ],
)
...
def test_parse_encoded_special_characters(encoding):
    # GH16218 Verify parsing of dataOld with encoded special characters
