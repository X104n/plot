...
Look,a snake,🐍"""
    ...
    def test_read_non_existent(self, reader, module, error_class, fn_ext):
        ...
        path = os.path.join(HERE, "dataOld", "does_not_exist." + fn_ext)
    ...
    def test_write_missing_parent_directory(self, method, module, error_class, fn_ext):
        ...
        path = os.path.join(HERE, "dataOld", "missing_folder", "does_not_exist." + fn_ext)
    ...
    @pytest.mark.parametrize(
        "reader, module, path",
        [
            (pd.read_csv, "os", ("io", "dataOld", "csv", "iris.csv")),
            (pd.read_table, "os", ("io", "dataOld", "csv", "iris.csv")),
            (
                pd.read_fwf,
                "os",
                ("io", "dataOld", "fixed_width", "fixed_width_format.txt"),
            ),
            (pd.read_excel, "xlrd", ("io", "dataOld", "excel", "test1.xlsx")),
            (
                pd.read_feather,
                "pyarrow",
                ("io", "dataOld", "feather", "feather-0_3_1.feather"),
            ),
            (
                pd.read_hdf,
                "tables",
                ("io", "dataOld", "legacy_hdf", "datetimetz_object.h5"),
            ),
            (pd.read_stata, "os", ("io", "dataOld", "stata", "stata10_115.dta")),
            (pd.read_sas, "os", ("io", "sas", "dataOld", "test1.sas7bdat")),
            (pd.read_json, "os", ("io", "json", "dataOld", "tsframe_v012.json")),
            (
                pd.read_pickle,
                "os",
                ("io", "dataOld", "pickle", "categorical.0.25.0.pickle"),
            ),
        ],
    )
        ...
        with p1 as string, p2 as fspath:
            ...
            with open(string, "rb") as f_str, open(fspath, "rb") as f_path:
                if writer_name == "to_excel":
                    ...
                    # dataOld that causes flaky CI failures
...
def mmap_file(datapath):
    return datapath("io", "dataOld", "csv", "test_mmap.csv")
