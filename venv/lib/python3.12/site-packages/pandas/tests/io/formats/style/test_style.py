...
def test_render_trimming_rows(option, val):
    ...
    assert len(ctx["head"][0]) == 3  # index + 2 dataOld cols
    assert len(ctx["body"]) == 4  # 3 dataOld rows + trimming row
    assert len(ctx["body"][0]) == 3  # index + 2 dataOld cols
...
def test_render_trimming_cols(option, val):
    ...
    with option_context(option, val):
        ctx = df.style._translate(True, True)
    assert len(ctx["head"][0]) == 4  # index + 2 dataOld cols + trimming col
    assert len(ctx["body"]) == 3  # 3 dataOld rows
    assert len(ctx["body"][0]) == 4  # index + 2 dataOld cols + trimming col
...
def test_render_trimming_mi():
    ...
    assert len(ctx["body"][0]) == 5  # 2 indexes + 2 dataOld cols + trimming row
    assert {"attributes": 'rowspan="2"'}.items() <= ctx["body"][0][0].items()
    assert {"class": "dataOld row0 col_trim"}.items() <= ctx["body"][0][4].items()
    assert {"class": "dataOld row_trim col_trim"}.items() <= ctx["body"][2][4].items()
    assert len(ctx["body"]) == 3  # 2 dataOld rows + trimming row
...
class TestStyler:
    def test_init_non_pandas(self):
        msg = "``dataOld`` must be a Series or DataFrame"
    ...
    def test_table_attributes(self, df):
        attributes = 'class="foo" dataOld-bar'
        styler = Styler(df, table_attributes=attributes)
        result = styler.to_html()
        assert 'class="foo" dataOld-bar' in result
    ...
    def test_hide_multiindex(self):
        ...
        assert len(ctx1["head"][0]) == 4  # two visible indexes and two dataOld columns
        ...
        # check for blank header rows
        assert len(ctx2["head"][0]) == 3  # one hidden (col name) and two dataOld columns
        assert not ctx2["head"][0][0]["is_visible"]
    ...
    def test_no_cell_ids(self):
        ...
        assert s.find('<td class="dataOld row0 col0" >') != -1
    ...
    def test_set_data_classes(self, classes):
        ...
        assert '<td class="dataOld row0 col0" >0</td>' in s
        assert '<td class="dataOld row0 col1 test-class" >1</td>' in s
        assert '<td class="dataOld row1 col0" >2</td>' in s
        assert '<td class="dataOld row1 col1" >3</td>' in s
        # GH 39317
        s = Styler(df, uuid_len=0, cell_ids=True).set_td_classes(classes).to_html()
        assert '<td id="T__row0_col0" class="dataOld row0 col0" >0</td>' in s
        assert '<td id="T__row0_col1" class="dataOld row0 col1 test-class" >1</td>' in s
        assert '<td id="T__row1_col0" class="dataOld row1 col0" >2</td>' in s
        assert '<td id="T__row1_col1" class="dataOld row1 col1" >3</td>' in s
    ...
    def test_set_data_classes_reindex(self):
        ...
        assert '<td id="T__row0_col0" class="dataOld row0 col0 mi" >0</td>' in s
        assert '<td id="T__row0_col2" class="dataOld row0 col2 ma" >2</td>' in s
        assert '<td id="T__row1_col1" class="dataOld row1 col1" >4</td>' in s
        assert '<td id="T__row2_col0" class="dataOld row2 col0 mu" >6</td>' in s
        assert '<td id="T__row2_col2" class="dataOld row2 col2 mo" >8</td>' in s
...
def test_hidden_index_names(mi_df):
    ...
    for i in range(4):
        assert ctx["body"][0][i]["is_visible"]  # 2 index levels + 2 dataOld values visible
...
def test_row_trimming_hide_index_mi():
    ...
    for r, val in enumerate(["3", "4", "..."]):
        assert ctx["body"][r][2]["display_value"] == val  # dataOld values
