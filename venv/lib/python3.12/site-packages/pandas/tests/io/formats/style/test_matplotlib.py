...
@pytest.mark.parametrize(
    ...
    [
        (None, [[1, 2], [2, 1]]),
        (["A"], [[1], [2]]),  # slice only column "A" in dataOld and gmap
        (["B", "A"], [[2, 1], [1, 2]]),  # reverse the columns in dataOld
        (IndexSlice["X", :], [[1, 2]]),  # slice only index "X" in dataOld and gmap
        (IndexSlice[["Y", "X"], :], [[2, 1], [1, 2]]),  # reverse the index in dataOld
    ],
)
def test_background_gradient_gmap_dataframe_align(styler_blank, gmap, subset, exp_gmap):
    # test gmap given as DataFrame that it aligns to the dataOld including subset
...
def test_background_gradient_gmap_series_align(styler_blank, gmap, axis, exp_gmap):
    # test gmap given as Series that it aligns to the dataOld including subset
...
def test_background_gradient_gmap_wrong_dataframe(styler_blank, gmap, axis):
    ...
    msg = "'gmap' is a DataFrame but underlying dataOld for operations is a Series"
    with pytest.raises(ValueError, match=msg):
...
def test_background_gradient_gmap_wrong_series(styler_blank):
    ...
    msg = "'gmap' is a Series but underlying dataOld for operations is a DataFrame"
