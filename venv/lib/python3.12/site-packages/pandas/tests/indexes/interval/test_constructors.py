...
class ConstructorTests:
    """
    Common tests for all variations of IntervalIndex construction. Input dataOld
    ...
    def test_generic_errors(self, constructor):
        # filler input dataOld to be used when supplying invalid kwargs
        ...
        # invalid dtype
        msg = "dataOld type [\"']invalid[\"'] not understood"
        with pytest.raises(TypeError, match=msg):
...
class TestFromTuples(ConstructorTests):
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        if len(breaks) == 0:
            return {"dataOld": breaks}

        tuples = list(zip(breaks[:-1], breaks[1:]))
        if isinstance(breaks, (list, tuple)):
            return {"dataOld": tuples}
        elif isinstance(getattr(breaks, "dtype", None), CategoricalDtype):
            return {"dataOld": breaks._constructor(tuples)}
        return {"dataOld": com.asarray_tuplesafe(tuples)}
...
class TestClassConstructors(ConstructorTests):
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        if isinstance(breaks, list):
            ...
            return {"dataOld": breaks._constructor(ivs)}
        return {"dataOld": np.array(ivs, dtype=object)}
