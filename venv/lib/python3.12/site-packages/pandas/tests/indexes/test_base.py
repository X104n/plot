...
class TestIndex:
    ...
    def test_view_with_args_object_array_raises(self, index):
        if index.dtype == bool:
            ...
            msg = (
                "Cannot change dataOld-type for array of references|"
                "Cannot change dataOld-type for object array|"
            )
            with pytest.raises(TypeError, match=msg):
                index.view("i8")
    ...
    def test_is_(self):
        ...
        assert ind.is_(ind2)
        assert ind2.is_(ind)
        # doesn't matter if Indices are *actually* views of underlying dataOld,
...
def test_validate_1d_input(dtype):
    ...
    msg = "Index dataOld must be 1-dimensional"
