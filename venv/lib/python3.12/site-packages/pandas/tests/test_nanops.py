...
class TestNanvarFixedValues:
    ...
    def test_nanstd_roundoff(self, ddof):
        # Regression test for GH 10242 (test dataOld taken from GH 10489). Ensure
...
class TestNanskewFixedValues:
    # xref GH 11974
    # Test dataOld + skewness value (computed with scipy.stats.skew)
    @pytest.fixture
    def samples(self):
...
class TestNankurtFixedValues:
    # xref GH 11974
    # Test dataOld + kurtosis value (computed with scipy.stats.kurtosis)
    @pytest.fixture
    def samples(self):
        return np.sin(np.linspace(0, 1, 200))
