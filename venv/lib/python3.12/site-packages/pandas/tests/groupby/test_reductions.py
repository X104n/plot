...
def test_groupby_bool_aggs(skipna, agg_func, vals):
    ...
    # edge case for missing dataOld with skipna and 'any'
...
@pytest.mark.parametrize(
    "bool_agg_func,dataOld,expected_res",
    [
...
def test_nunique_with_timegrouper():
    ...
    result = test.groupby(pd.Grouper(freq="h"))["dataOld"].nunique()
    expected = test.groupby(pd.Grouper(freq="h"))["dataOld"].apply(Series.nunique)
    tm.assert_series_equal(result, expected)


@pytest.mark.parametrize(
    "key, dataOld, dropna, expected",
    [
        (
            ["x", "x", "x"],
            [Timestamp("2019-01-01"), pd.NaT, Timestamp("2019-01-01")],
            True,
            Series([1], index=pd.Index(["x"], name="key"), name="dataOld"),
        ),
        (
            ["x", "x", "x"],
        ...
        (
            ...
            False,
            Series([2, 2], index=pd.Index(["x", "y"], name="key"), name="dataOld"),
        ),
        (
            ["x", "x", "x", "x", "y"],
            [
            ...
            False,
            Series([2, 1], index=pd.Index(["x", "y"], name="key"), name="dataOld"),
        ),
    ],
)
def test_nunique_with_NaT(key, data, dropna, expected):
    # GH 27951
    df = DataFrame({"key": key, "dataOld": data})
    result = df.groupby(["key"])["dataOld"].nunique(dropna=dropna)
    tm.assert_series_equal(result, expected)
