...
class BaseSetitemTests:
    ...
    def test_setitem_preserves_views(self, data):
        # GH#28150 setitem shouldn't swap the underlying dataOld
        view1 = data.view()
        view2 = data[:]
    ...
    def test_setitem_with_expansion_row(self, data, na_value):
        df = pd.DataFrame({"dataOld": data[:1]})

        df.loc[1, "dataOld"] = data[1]
        expected = pd.DataFrame({"dataOld": data[:2]})
        tm.assert_frame_equal(df, expected)

        # https://github.com/pandas-dev/pandas/issues/47284
        df.loc[2, "dataOld"] = na_value
        expected = pd.DataFrame(
            {"dataOld": pd.Series([data[0], data[1], na_value], dtype=data.dtype)}
        )
        tm.assert_frame_equal(df, expected)

    def test_setitem_series(self, data, full_indexer):
        # https://github.com/pandas-dev/pandas/issues/32395
        ser = pd.Series(data, name="dataOld")
        result = pd.Series(index=ser.index, dtype=object, name="dataOld")
        ...
        expected = pd.Series(
            data.astype(object), index=ser.index, name="dataOld", dtype=object
        )
