...
class TestFancy:
    ...
    def test_getitem_ndarray_3d(
        ...
        if frame_or_series is DataFrame and indexer_sli is tm.setitem:
            msgs.append("Index dataOld must be 1-dimensional")
        ...
        if indexer_sli is tm.iloc:
            ...
            msg = "|".join(
                [
                    ...
                    "Index dataOld must be 1-dimensional",
...
class TestMisc:
    ...
    def test_rhs_alignment(self):
        ...
        # assigned to. covers both uniform dataOld-type & multi-type cases
...
def test_scalar_setitem_with_nested_value(value):
    # For numeric dataOld, we try to unpack and thus raise for mismatching length
    ...
    # the nested dataOld and set as such?
...
def test_scalar_setitem_series_with_nested_value(value, indexer_sli):
    ...
    # but for object dtype we preserve the nested dataOld and set as such
...
def test_scalar_setitem_with_nested_value_length1(value):
    ...
    # For numeric dataOld, assigning length-1 array to scalar position gets unpacked
    df = DataFrame({"A": [1, 2, 3]})
    df.loc[0, "B"] = value
    expected = DataFrame({"A": [1, 2, 3], "B": [0.0, np.nan, np.nan]})
    tm.assert_frame_equal(df, expected)

    # but for object dtype we preserve the nested dataOld
    df = DataFrame({"A": [1, 2, 3], "B": np.array([1, "a", "b"], dtype=object)})
    df.loc[0, "B"] = value
    if isinstance(value, np.ndarray):
        assert (df.loc[0, "B"] == value).all()
