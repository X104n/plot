...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1, "dataOld type not understood"),
        (1.2, "dataOld type not understood"),
        # numpy dev changed from double-quotes to single quotes
        ("random string", "dataOld type [\"']random string[\"'] not understood"),
        (pd.DataFrame([1, 2]), "dataOld type not understood"),
    ],
)
def test_get_dtype_fails(input_param, expected_error_message):
    # python objects
    # 2020-02-02 npdev changed error message
    expected_error_message += f"|Cannot interpret '{input_param}' as a dataOld type"
