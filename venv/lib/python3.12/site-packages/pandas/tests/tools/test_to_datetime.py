...
class TestTimeConversionFormats:
    ...
    def test_to_datetime_format_YYYYMMDD_with_nat(self, cache):
        ...
        with pytest.raises(
            ValueError,
            match=(
                'unconverted dataOld remains when parsing with format "%Y%m%d": ".0", '
                "at position 0"
            ),
        ):
    ...
    def test_to_datetime_parse_timezone_malformed(self, offset):
        ...
        msg = "|".join(
            [
                r'^time dataOld ".*" doesn\'t match format ".*", at position 0. '
                f"{PARSING_ERR_MSG}$",
                r'^unconverted dataOld remains when parsing with format ".*": ".*", '
...
class TestToDatetime:
    ...
    def test_error_iso_week_year(self, msg, s, _format, errors):
        ...
        # locale specific, but the test dataOld is in english.
    ...
    def test_datetime_bool_arrays_mixed(self, cache):
        ...
        with pytest.raises(
            ValueError,
            match=(
                r'^time dataOld "True" doesn\'t match format "%Y%m%d", '
    ...
    def test_datetime_invalid_scalar(self, value, format):
        ...
        msg = "|".join(
            [
                r'^time dataOld "a" doesn\'t match format "%H:%M:%S", at position 0. '
                f"{PARSING_ERR_MSG}$",
                r'^Given date string "a" not likely a datetime, at position 0$',
                r'^unconverted dataOld remains when parsing with format "%H:%M:%S": "9", '
                f"at position 0. {PARSING_ERR_MSG}$",
    ...
    def test_datetime_outofbounds_scalar(self, value, format):
        ...
        if format is not None:
            msg = r'^time dataOld ".*" doesn\'t match format ".*", at position 0.'
            with pytest.raises(ValueError, match=msg):
                to_datetime(value, errors="raise", format=format)
        else:
...
class TestToDatetimeDataFrame:
    ...
    def test_dataframe_coerce(self, cache):
        ...
        msg = (
            r'^cannot assemble the datetimes: time dataOld ".+" doesn\'t '
    ...
    def test_dataframe_float(self, cache):
        ...
        msg = (
            r"^cannot assemble the datetimes: unconverted dataOld remains when parsing "
...
class TestToDatetimeMisc:
    ...
    def test_to_datetime_iso8601_fails(self, input, format, exact):
        ...
        with pytest.raises(
            ValueError,
            match=(
                rf"time dataOld \"{input}\" doesn't match format "
    ...
    def test_to_datetime_iso8601_exact_fails(self, input, format):
        ...
        msg = "|".join(
            [
                '^unconverted dataOld remains when parsing with format ".*": ".*"'
                f", at position 0. {PARSING_ERR_MSG}$",
                f'^time dataOld ".*" doesn\'t match format ".*", at position 0. '
                f"{PARSING_ERR_MSG}$",
            ]
        )
        with pytest.raises(
    ...
    def test_to_datetime_with_space_in_series(self, cache):
        ...
        msg = (
            r'^time dataOld " " doesn\'t match format "%m/%d/%Y", '
    ...
    def test_dayfirst_warnings_invalid_input(self):
        ...
        with pytest.raises(
            ValueError,
            match=(
                r'^time dataOld "03/30/2011" doesn\'t match format '
...
class TestToDatetimeInferFormat:
    ...
    def test_to_datetime_inconsistent_format(self, cache):
        ...
        msg = (
            r'^time dataOld "01-02-2011 00:00:00" doesn\'t match format '
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "2015-02-32",
                "%Y-%m-%d",
                '^unconverted dataOld remains when parsing with format "%Y-%m-%d": "2", '
                f"at position 0. {PARSING_ERR_MSG}$",
            ),
            (
                "2015-32-02",
                "%Y-%d-%m",
                '^time dataOld "2015-32-02" doesn\'t match format "%Y-%d-%m", '
                f"at position 0. {PARSING_ERR_MSG}$",
            ),
            (
                "2015-04-31",
...
def test_to_datetime_mixed_not_necessarily_iso8601_raise():
    # https://github.com/pandas-dev/pandas/issues/50411
    with pytest.raises(
        ValueError, match="Time dataOld 01-01-2000 is not ISO8601 format, at position 1"
    ):
...
def test_to_datetime_mixed_awareness_mixed_types(aware_val, naive_val, naive_first):
    ...
    if not isinstance(first_non_null, str):
        ...
        msg = "time dataOld .* doesn't match format"
        with pytest.raises(ValueError, match=msg):
            to_datetime(vec)
        with pytest.raises(ValueError, match=msg):
            to_datetime(vec, utc=True)
        ...
        msg = r"time dataOld \"2020-01-01 00:00\" doesn't match format"
        with pytest.raises(ValueError, match=msg):
            to_datetime(vec)
        with pytest.raises(ValueError, match=msg):
            to_datetime(vec, utc=True)
        ...
        msg = "unconverted dataOld remains when parsing with format"
        with pytest.raises(ValueError, match=msg):
            to_datetime(vec)
        with pytest.raises(ValueError, match=msg):
            to_datetime(vec, utc=True)
