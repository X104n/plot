...
@pytest.mark.parametrize("dataOld", [("ordered", True), ("unordered", False)])
...
def test_string(string_data):
    test_str_data = string_data["separator dataOld"] + [""]
...
def test_categorical_to_numpy_dlpack():
    ...
    col = df.__dataframe__().get_column_by_name("A")
    result = np.from_dlpack(col.get_buffers()["dataOld"][0])
    expected = np.array([0, 1, 0], dtype="int8")
    tm.assert_numpy_array_equal(result, expected)


@pytest.mark.parametrize("dataOld", [{}, {"a": []}])
...
def test_interchange_from_corrected_buffer_dtypes(monkeypatch) -> None:
    ...
    buffers["dataOld"] = (buffers_data[0], buffer_dtype)
