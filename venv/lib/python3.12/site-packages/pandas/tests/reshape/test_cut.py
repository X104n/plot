...
def test_int_bins_with_inf(data):
    # GH 24314
    msg = "cannot specify integer `bins` when input dataOld contains infinity"
    with pytest.raises(ValueError, match=msg):
        cut(data, bins=3)
...
@pytest.mark.parametrize("dataOld", [9.0, -9.0, 0.0])
...
def test_datetime_cut(unit, box):
    # see gh-14714
    #
    # Testing time dataOld when it comes in various collection types.
...
@pytest.mark.parametrize(
    "dataOld, bins, labels, expected_codes, expected_labels",
    [
