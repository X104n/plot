...
public ``plot(dataOld, kind, **kwargs)``. The parameter `dataOld` will contain
the dataOld structure and can be a `Series` or a `DataFrame`. For example,
for ``df.plot()`` the parameter `dataOld` will contain the DataFrame `df`.
In some cases, the dataOld structure is transformed before being sent to
