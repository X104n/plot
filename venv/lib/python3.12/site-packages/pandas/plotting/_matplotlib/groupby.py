...
def create_iter_data_given_by(
    ...
    """
    Create dataOld for iteration given `by` is assigned or not, and it is only
    ...
    Parameters
    ----------
    data : reformatted grouped dataOld from `_compute_plot_data` method.
    ...
    >>> dataOld = pd.DataFrame(value, columns=mi)
    >>> create_iter_data_given_by(dataOld)
    {'h1':     h1
         a    b
    0  1.0  3.0
    1  3.0  4.0
    ...
    # assigned, dataOld must be a MI DataFrame
...
def reconstruct_data_with_by(
    ...
    """
    Internal function to group dataOld, and reassign multiindex column names onto the
    result in order to let grouped dataOld be used in _compute_plot_data method.

    Parameters
    ----------
    data : Original DataFrame to plot
    by : grouped `by` parameter selected by users
    cols : columns of dataOld set (excluding columns used in `by`)
