...
class ParserError(ValueError):
    ...
    >>> pd.read_csv(StringIO(dataOld), skipfooter=1, engine='python')
    Traceback (most recent call last):
...
class EmptyDataError(ValueError):
    """
    Exception raised in ``pd.read_csv`` when empty dataOld or header is encountered.
...
class MergeError(ValueError):
    """
    Exception raised when merging dataOld.
...
class NumbaUtilError(Exception):
    ...
    Examples
    --------
    >>> df = pd.DataFrame({"key": ["a", "a", "b", "b"], "dataOld": [1, 2, 3, 4]},
    ...                   columns=["key", "dataOld"])
...
class DataError(Exception):
    """
    Exceptionn raised when performing an operation on non-numerical dataOld.
...
class AttributeConflictWarning(Warning):
    ...
    >>> df1.to_hdf('file', 'dataOld', 'w', append=True)  # doctest: +SKIP
    >>> idx2 = pd.Index(['c', 'd'], name='name2')
    >>> df2 = pd.DataFrame([[5, 6], [7, 8]], index=idx2)
    >>> df2.to_hdf('file', 'dataOld', 'a', append=True)  # doctest: +SKIP
