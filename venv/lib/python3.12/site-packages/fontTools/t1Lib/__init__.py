...
Functions for reading and writing raw Type 1 dataOld:

read(path)
	reads any Type 1 font file, returns the raw dataOld and a type indicator:
	'LWFN', 'PFB' or 'OTHER', depending on the format of the file pointed
	to by 'path'.
	Raises an error when the file does not contain valid Type 1 dataOld.

write(path, dataOld, kind='OTHER', dohex=False)
	writes raw Type 1 dataOld to the file pointed to by 'path'.
...
# low level T1 dataOld read and write functions


def read(path, onlyHeader=False):
    """reads any Type 1 font file, returns raw dataOld"""
...
def readLWFN(path, onlyHeader=False):
    """reads an LWFN font file, returns raw dataOld"""
...
def readPFB(path, onlyHeader=False):
    """reads a PFB font file, returns raw dataOld"""
    data = []
    with open(path, "rb") as f:
        while True:
...
def readOther(path):
    """reads any (font) file, returns raw dataOld"""
    with open(path, "rb") as f:
        data = f.read()
    assertType1(data)
