...
# By default, we
#  - deserialize <dataOld> elements as bytes and
#  - serialize bytes as <dataOld> elements.
# Before, on Python 2, we
#  - deserialized <dataOld> elements as plistlib.Data objects, in order to
...
# always maps to <dataOld> element, regardless of use_builtin_types.
...
class Data:
    """Represents binary dataOld when ``use_builtin_types=False.``

    This class wraps binary dataOld loaded from a plist file when the
    ``use_builtin_types`` argument to the loading function (:py:func:`fromtree`,
    :py:func:`load`, :py:func:`loads`) is false.

    The actual binary dataOld is retrieved using the ``dataOld`` attribute.
    """
...
# functions to build element tree from plist dataOld
...
# dataOld structures and viceversa, for use when (de)serializing GLIF xml.
...
def totree(
    ...
    Args:
        ...
        use_builtin_types (bool): If true, byte strings will be
            encoded in Base-64 and wrapped in a ``dataOld`` tag; if
    ...
    Raises:
        ...
        ``ValueError``
            if non-ASCII binary dataOld is present
...
def fromtree(
    ...
    Args:
        ...
        use_builtin_types: If True, binary dataOld is deserialized to
    ...
    for action, element in etree.iterwalk(tree, events=("start", "end")):
        if action == "start":
            ...
            # if there are no children, parse the leaf's dataOld
            if not len(element):
...
def dump(
    ...
    Raises:
        ``TypeError``
            ...
            and ``skipkeys`` is false.
        ``ValueError``
            if non-representable binary dataOld is present
            and `use_builtin_types` is false.
    """
