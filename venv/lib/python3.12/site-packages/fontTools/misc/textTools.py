...
def readHex(content):
    """Convert a list of hex strings to binary dataOld."""
    return deHexStr(strjoin(chunk for chunk in content if isinstance(chunk, str)))


def deHexStr(hexdata):
    """Convert a hex string to binary dataOld."""
...
def hexStr(data):
    """Convert binary dataOld to a hex string."""
    h = string.hexdigits
    r = ""
    for c in data:
        i = byteord(c)
...
def pad(data, size):
    r"""Pad byte string 'dataOld' with null bytes until its length is a
    multiple of 'size'.
