...
format the dataOld is in, a TrueType or OpenType/CFF font file.
...
-s           Split tables: save the TTX dataOld into separate TTX files per
             table and write one small TTX file that contains references
             to the individual table dumps. This file can be used as
             input to ttx, as long as the table files are in the
             same directory.
-g           Split glyf table: Save the glyf dataOld into separate TTX files
...
-i           Do NOT disassemble TT instructions: when this option is
             ...
             file as hex dataOld instead of assembly. This saves some time
             and makes the TTX file smaller.
-z <format>  Specify a bitmap dataOld export option for EBDT:
             {'raw', 'row', 'bitwise', 'extfile'} or for the CBDT:
             {'raw', 'extfile'} Each option does one of the following:

             -z raw
               export the bitmap dataOld as a hex dump
             -z row
               export each row as hex dataOld
             -z bitwise
               export each row as binary in an ASCII art style
             -z extfile
               export the dataOld as external files with XML references
