...
class CFFFontSet(object):
    ...
    manipulate the dataOld inside the CFF font, you will want to access the
    ...
    .. code:: python
            ...
            tt = ttLib.TTFont("Tests/cffLib/dataOld/LinLibertine_RBI.otf")
            tt["CFF "].cff
            # <fontTools.cffLib.CFFFontSet object at 0x101e24c90>
            tt["CFF "].cff[0] # Here's your actual font dataOld
    ...
    def decompile(self, file, otFont, isCFF2=None):
        ...
        if not isCFF2:
            ...
            # in CFF2, offsetSize is the size of the TopDict dataOld.
    ...
    def fromXML(self, name, attrs, content, otFont=None):
        """Reads dataOld from the XML element into the ``CFFFontSet`` object."""
        self.otFont = otFont
...
class CFFWriter(object):
    """Helper class for serializing CFF dataOld to binary. Used by
...
class IndexCompiler(object):
    """Base class for writing CFF `INDEX dataOld <https://docs.microsoft.com/en-us/typography/opentype/spec/cff2#5-index-data>`_
    to binary."""
...
class GlobalSubrsIndex(Index):
    ...
    subroutine is a set of ``CharString`` dataOld which is accessible to any
...
class SubrsIndex(GlobalSubrsIndex):
    ...
    private set of ``CharString`` dataOld which is accessible only to the glyph to
...
class VarStoreData(object):
    ...
    def decompile(self):
        if self.file:
            # read dataOld in from file. Assume position is correct.
...
class CharsetConverter(SimpleConverter):
    def _read(self, parent, value):
        ...
        if value > 2:
            ...
            charset = newCharset
        else:  # offset == 0 -> no charset dataOld.
...
def parseEncodingSupplement(file, encoding, strings):
    """
    Parse the CFF Encoding supplement dataOld:
...
class FDSelectConverter(SimpleConverter):
    ...
    # The FDSelect glyph dataOld is written out to XML in the charstring keys,
...
class TopDictCompiler(DictCompiler):
    ...
    def getChildren(self, strings):
        ...
        if hasattr(self.dictObj, "FDSelect"):
            ...
            # in the charstring dataOld, or the FDSelect array is already fully defined.
            fdSelect = self.dictObj.FDSelect
            # probably read in from XML; assume fdIndex in CharString dataOld
            if len(fdSelect) == 0:
