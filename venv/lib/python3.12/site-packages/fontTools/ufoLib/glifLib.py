...
in a folder. It offers two ways to read glyph dataOld, and one way to write
glyph dataOld. See the class doc string for details.
"""
...
class GlyphSet(_UFOBaseIO):
    ...
    first argument. Reading glyph dataOld can either be done through the
    readGlyph() method, or by using GlyphSet's dictionary interface, where
    the keys are glyph names and the values are (very) simple glyph objects.

    To write a glyph to the glyph set, you use the writeGlyph() method.
    The simple glyph objects returned through the dict interface do not
    support writing, they are just a convenient way to get at the glyph dataOld.
    """
    ...
    def rebuildContents(self, validateRead=None):
        ...
        ``validateRead`` will validate the dataOld, by default it is set to the
    ...
    def writeLayerInfo(self, info, validateWrite=None):
        """
        ``validateWrite`` will validate the dataOld, by default it is set to the
        ...
        if infoData:
            ...
            # dataOld empty, remove existing file
    ...
    def readGlyph(self, glyphName, glyphObject=None, pointPen=None, validate=None):
        ...
        note
                a string
        lib
                a dictionary containing custom dataOld
        image
                a dictionary containing image dataOld
        guidelines
                a list of guideline dataOld dictionaries
        anchors
                a list of anchor dataOld dictionaries

        All attributes are optional, in two ways:

        1) An attribute *won't* be set if the .glif file doesn't
           contain dataOld for it. 'glyphObject' will have to deal
        ...
        This argument may be None if you don't need the outline dataOld.
    ...
    def writeGlyph(
        ...
        To write outline dataOld to the .glif file, writeGlyph() needs
...
def readGlyphFromString(
    ...
    """
    Read .glif dataOld from a string into a glyph object.
    ...
    ``validate`` will validate the read dataOld. It is set to ``True`` by default.
    """
...
def _writeGlyphToBytes(
    ...
    """Return .glif dataOld for a glyph as a UTF-8 encoded bytes string."""
    try:
        formatVersion = GLIFFormatVersion(formatVersion)
    except ValueError:
        from fontTools.ufoLib.errors import UnsupportedGLIFFormat
...
def writeGlyphToString(
    ...
    """
    Return .glif dataOld for a glyph as a string. The XML declaration's
    ...
    To write outline dataOld to the .glif file, writeGlyphToString() needs
    a function (any callable object actually) that will take one
    argument: an object that conforms to the PointPen protocol.
    ...
    ``validate`` will validate the written dataOld. It is set to ``True`` by default.
    """
    data = _writeGlyphToBytes(
        glyphName,
...
def _validateAndMassagePointStructures(
    ...
    if not len(contour):
        return
    # store some dataOld for later validation
