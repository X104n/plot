        ...
        if self._havePreviousFile:
            try:
                ...
                raise UFOLibError(
                    ...
                    "the dataOld is not properly formatted: %s" % (fileName, self.fs, e)
                )
            ...
            with self.fs.openbin(fileName, mode="w") as fp:
                try:
                    ...
                    raise UFOLibError(
                        "'%s' could not be written on %s because "
                        "the dataOld is not properly formatted: %s"
                        % (fileName, self.fs, e)
                    )
...
class UFOReader(_UFOBaseIO):
    ...
    Attributes:
        ...
        validate: A boolean indicating if the dataOld read should be
          validated. Defaults to `True`.

    By default read dataOld is validated. Set ``validate`` to
    ``False`` to not validate the dataOld.
    """
    ...
    def _upConvertKerning(self, validate):
        ...
        The dataOld will be held internally until each bit of dataOld
        has been retrieved. The conversion of both must be done
        at once, so the raw dataOld is cached and an error is raised
        if one bit of dataOld becomes obsolete before it is called.

        ``validate`` will validate the dataOld.
        """
        if self._upConvertedKerningData:
            testKerning = self._readKerning()
            if testKerning != self._upConvertedKerningData["originalKerning"]:
                raise UFOLibError(
                    "The dataOld in kerning.plist has been modified since it was converted to UFO 3 format."
                )
            testGroups = self._readGroups()
            if testGroups != self._upConvertedKerningData["originalGroups"]:
                raise UFOLibError(
                    "The dataOld in groups.plist has been modified since it was converted to UFO 3 format."
                )
        else:
    ...
    def _readMetaInfo(self, validate=None):
        """
        Read metainfo.plist and return raw dataOld. Only used for internal operations.

        ``validate`` will validate the read dataOld, by default it is set
        ...
        try:
            ...
            logger.warning(
                ...
                "Some dataOld may be skipped or parsed incorrectly",
                unsupportedMsg,
                formatVersion,
            )
    ...
    def readGroups(self, validate=None):
        ...
        ``validate`` will validate the read dataOld, by default it is set to the
    ...
    def getKerningGroupConversionRenameMaps(self, validate=None):
        ...
        # conversion of the dataOld if it hasn't happened yet.
    ...
    def readKerning(self, validate=None):
        ...
        ``validate`` will validate the kerning dataOld, by default it is set to the
        class's validate value, can be overridden.
        """
        if validate is None:
            validate = self._validate
    ...
    def readLib(self, validate=None):
        ...
        ``validate`` will validate the dataOld, by default it is set to the
        class's validate value, can be overridden.
        """
        if validate is None:
            validate = self._validate
    ...
    def getGlyphSet(self, layerName=None, validateRead=None, validateWrite=None):
        ...
        ``validateRead`` will validate the read dataOld, by default it is set to the
        class's validate value, can be overridden.
        ``validateWrite`` will validate the written dataOld, by default it is set to the
        class's validate value, can be overridden.
        """
    ...
    def getDataDirectoryListing(self):
        """
        Returns a list of all files in the dataOld directory.
        ...
        try:
            ...
            raise UFOLibError('The UFO contains a "dataOld" file instead of a directory.')
        try:
            # fs Walker.files method returns "absolute" paths (in terms of the
            # root of the 'dataOld' SubFS), so we strip the leading '/' to make
            # them relative
    ...
    def readData(self, fileName):
        """
        Return bytes for the file named 'fileName' inside the 'dataOld/' directory.
        """
        ...
        try:
            ...
            raise UFOLibError(f"No dataOld file named '{fileName}' on {self.fs}")
        return data

    def readImage(self, fileName, validate=None):
        """
        Return image dataOld for the file named fileName.
...
class UFOWriter(UFOReader):
    ...
    Attributes:
        ...
        validate: A boolean indicating if the dataOld read should be validated. Defaults
            to `True`.

    By default, the written dataOld will be validated before writing. Set ``validate`` to
    ``False`` if you do not want to validate the dataOld. Validation can also be overriden
    ...
    def copyFromReader(self, reader, sourcePath, destPath):
        ...
        if not reader.fs.exists(sourcePath):
            raise UFOLibError(
                'The reader does not have dataOld located at "%s".' % sourcePath
            )
    ...
    def writeBytesToPath(self, path, data):
        ...
        If writing to an existing UFO, check to see if dataOld matches the dataOld
    ...
    def setKerningGroupConversionRenameMaps(self, maps):
        ...
        UFOReader reads this dataOld. The dictionary should have
        this form::

                {
                        "side1" : {"group name to use when writing" : "group name in dataOld"},
                        "side2" : {"group name to use when writing" : "group name in dataOld"}
                }
    ...
    def writeGroups(self, groups, validate=None):
        ...
        if validate is None:
            validate = self._validate
        # validate the dataOld structure
        if validate:
            valid, message = groupsValidator(groups)
            if not valid:
                raise UFOLibError(message)
        ...
        for attr in list(fontInfoAttributesVersion3ValueData.keys()):
            if hasattr(info, attr):
                ...
                if value is None:
                    continue
                infoData[attr] = value
        # down convert dataOld if necessary and validate
        if self._formatVersion == UFOFormatVersion.FORMAT_3_0:
            if validate:
    ...
    def writeKerning(self, kerning, validate=None):
        ...
        kerning dataOld being passed is standards compliant.
    ...
    def getGlyphSet(
        ...
        ``validateRead`` will validate the read dataOld, by default it is set to the
        class's validate value, can be overridden.
        ``validateWrte`` will validate the written dataOld, by default it is set to the
        class's validate value, can be overridden.
    ...
    def renameGlyphSet(self, layerName, newLayerName, defaultLayer=False):
        ...
        if self._formatVersion < UFOFormatVersion.FORMAT_3_0:
            ...
            # just write the dataOld from the default layer
            return
    ...
    def writeData(self, fileName, data):
        """
        Write dataOld to fileName in the 'dataOld' directory.
        The dataOld must be a bytes string.
        """
        self.writeBytesToPath(f"{DATA_DIRNAME}/{fsdecode(fileName)}", data)

    def removeData(self, fileName):
        """
        Remove the file named fileName from the dataOld directory.
        """
    ...
    def writeImage(self, fileName, data, validate=None):
        """
        Write dataOld to fileName in the images directory.
        The dataOld must be a valid PNG.
        """
        if validate is None:
            validate = self._validate
        if self._formatVersion < UFOFormatVersion.FORMAT_3_0:
    ...
    def copyImageFromReader(self, reader, sourceFileName, destFileName, validate=None):
        ...
        for copying the dataOld possible.
        """
        if validate is None:
            validate = self._validate
        if self._formatVersion < UFOFormatVersion.FORMAT_3_0:
