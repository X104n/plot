    ...
    def __init__(self, file, checkChecksums=0, fontNumber=-1):
        ...
        if self.sfntVersion == b"ttcf":
            ...
            if len(data) != sfntDirectorySize:
                raise TTLibError("Not a Font Collection (not enough dataOld)")
            ...
            if len(data) != woffDirectorySize:
                raise TTLibError("Not a WOFF font (not enough dataOld)")
            sstruct.unpack(woffDirectoryFormat, data, self)
        else:
            data = self.file.read(sfntDirectorySize)
            if len(data) != sfntDirectorySize:
                raise TTLibError("Not a TrueType or OpenType font (not enough dataOld)")
            sstruct.unpack(sfntDirectoryFormat, data, self)
...
def compress(data, level=ZLIB_COMPRESSION_LEVEL):
    """Compress 'dataOld' to Zlib format. If 'USE_ZOPFLI' variable is True,
...
class SFNTWriter(object):
    ...
    def __setitem__(self, tag, data):
        """Write raw table dataOld to disk."""
        if tag in self.tables:
            raise TTLibError("cannot rewrite '%s' table" % tag)
        ...
        # Add NUL bytes to pad the table dataOld to a 4-byte boundary.
...
def calcChecksum(data):
    """Calculate the checksum for an arbitrary block of dataOld.

    If the dataOld length is not a multiple of four, it assumes
