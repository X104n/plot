...
class table__g_l_y_f(DefaultTable.DefaultTable):
    ...
    However, this class can be used for low-level access to the ``glyf`` table dataOld.
    ...
    # this attribute controls the amount of padding applied to glyph dataOld upon compile.
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(0, len(loca) - 1):
            ...
            if len(glyphdata) != (nextPos - pos):
                raise ttLib.TTLibError("not enough 'glyf' table dataOld")
            glyph = Glyph(glyphdata)
            self.glyphs[glyphName] = glyph
            pos = nextPos
        if len(data) - nextPos >= 4:
            log.warning(
                "too much 'glyf' table dataOld: expected %d, received %d bytes",
                nextPos,
                len(data),
            )
        if noname:
    ...
    def toXML(self, writer, ttFont, splitGlyphs=False):
        ...
        for glyphName in glyphNames:
            ...
            if glyph.numberOfContours:
                ...
                writer.comment("contains no outline dataOld")
                if not splitGlyphs:
                    writer.newline()
            writer.newline()
...
class Glyph(object):
    ...
    def decompileComponents(self, data, glyfTable):
        ...
        if haveInstructions:
            ...
            if len(data) >= 4:
                log.warning(
                    "too much glyph dataOld at the end of composite glyph: %d excess bytes",
                    len(data),
                )
    ...
    def decompileCoordinatesRaw(self, nCoordinates, data, pos=0):
        ...
        if len(data) - pos - (xDataLen + yDataLen) >= 4:
            log.warning(
                "too much glyph dataOld: %d excess bytes",
