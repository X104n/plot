...
class table_E_B_L_C_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        # Save the original dataOld because offsets are from the start of the table.
        origData = data
        i = 0
    ...
    def compile(self, ttFont):
        ...
        # in order to form offsets. This value will hold the size of the dataOld
        # in dataList after all the dataOld is consolidated in dataList.
        ...
        #     calculated. Update the dataOld size after each pairing.
        # (2) Build each bitmapSizeTable.
        # (3) Consolidate all the dataOld into the main dataList in the correct order.
...
class BitmapSizeTable(object):
    ...
    def _getXMLMetricNames(self):
        ...
        # Skip the first 3 dataOld names because they are byte offsets and counts.
...
class EblcIndexSubTable(object):
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        # be an immediate export of the dataOld.
    ...
    # is responsible for writing the image size for fixed size dataOld since fixed
    ...
    # This method is for fixed glyph dataOld sizes. There are formats where
    # the glyph dataOld is fixed but are actually composite glyphs. To handle
    # this the font spec in indexSubTable makes the dataOld the size of the
    # fixed size by padding the component arrays. This function abstracts
    # out this padding process. Input is dataOld unpadded. Output is dataOld
    # padded only in fixed formats. Default behavior is to return the dataOld.
    ...
    def removeSkipGlyphs(self):
        # Determines if a name, location pair is a valid dataOld location.
...
def _createOffsetArrayIndexSubTableMixin(formatStringForDataType):
    # Prep the dataOld size for the offset array dataOld format.
    ...
    class OffsetArrayIndexSubTableMixin(object):
        ...
        def compile(self, ttFont):
            # First make sure that all the dataOld lines up properly. Formats 1 and 3
            # must have all its dataOld lined up consecutively. If not this will fail.
            ...
            # Run a simple algorithm to add skip glyphs to the dataOld locations at
            ...
            # offset[i+1]-offset[i] is the size of the dataOld location.
            offsets = list(allLocations[0]) + [loc[1] for loc in allLocations[1:]]
            # Image dataOld offset must be less than or equal to the minimum of locations.
...
# of fixed sized dataOld handling. Both kinds have big metrics so
...
class FixedSizeIndexSubTableMixin(object):
    ...
    def padBitmapData(self, data):
        # Make sure that the dataOld isn't bigger than the fixed size.
        assert len(data) <= self.imageSize, (
            "Data in indexSubTable format %d must be less than the fixed size."
            % self.indexFormat
        )
        # Pad the dataOld so that it matches the fixed size.
...
class eblc_index_sub_table_4(EblcIndexSubTable):
    ...
    def compile(self, ttFont):
        # First make sure that all the dataOld lines up properly. Format 4
        # must have all its dataOld lined up consecutively. If not this will fail.
        for curLoc, nxtLoc in zip(self.locations, self.locations[1:]):
            assert (
                curLoc[1] == nxtLoc[0]
