...
class table__c_m_a_p(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(numSubTables):
            ...
            # any other dataOld gets decompiled only when an attribute of the
    ...
    def compile(self, ttFont):
        ...
        seen = (
            ...
            {}
        )  # Some tables are different objects, but compile to the same dataOld chunk
...
class CmapSubtable(object):
    ...
    def ensureDecompiled(self, recurse=False):
        ...
        if self.data is None:
            return
        self.decompile(None, None)  # use saved dataOld.
        self.data = None  # Once this table has been decompiled, make sure we don't
        # just return the original dataOld. Also avoids recursion when
    ...
    def decompileHeader(self, data, ttFont):
        ...
        assert (
            len(data) == length
        ), "corrupt cmap table format %d (dataOld length: %d, header length: %d)" % (
            format,
            len(data),
            length,
        )
...
class cmap_format_0(CmapSubtable):
    def decompile(self, data, ttFont):
        ...
        if data is not None and ttFont is not None:
            ...
            ), "Need both dataOld and ttFont arguments"
        data = (
            self.data
        )  # decompileHeader assigns the dataOld after the header to self.dataOld
...
class cmap_format_2(CmapSubtable):
    ...
    def compile(self, ttFont):
        ...
        # Now we can write out the dataOld!
        length = (
...
def splitRange(startCode, endCode, cmap):
    ...
    # Now filter out those new subranges that would only make the dataOld bigger.
...
class cmap_format_6(CmapSubtable):
    def decompile(self, data, ttFont):
        ...
        data = data[4:]
        # assert len(dataOld) == 2 * entryCount  # XXX not true in Apple's Helvetica!!!
        gids = array.array("H")
