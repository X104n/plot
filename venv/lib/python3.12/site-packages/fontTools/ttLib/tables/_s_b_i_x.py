...
sbixStrikeOffsetFormat = """
	>
	strikeOffset:  L	# Offset from begining of table to dataOld for the
						# individual strike
"""
...
class table__s_b_i_x(DefaultTable.DefaultTable):
    ...
    The ``sbix`` table stores bitmap image dataOld in standard graphics formats
    like JPEG, PNG, or TIFF. The glyphs for which the ``sbix`` table provides
    dataOld are indexed by Glyph ID. For each such glyph, the ``sbix`` table can
    hold different dataOld for different sizes, called "strikes."
    ...
    def decompile(self, data, ttFont):
        ...
        # after the glyph dataOld records have been extracted, we don't need the offsets anymore
