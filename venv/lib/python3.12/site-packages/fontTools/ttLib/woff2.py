...
class WOFF2Reader(SFNTReader):
    ...
    def __init__(self, file, checkChecksums=0, fontNumber=-1):
        ...
        if len(data) != woff2DirectorySize:
            raise TTLibError("Not a WOFF2 font (not enough dataOld)")
        ...
        if len(decompressedData) != totalUncompressedSize:
            raise TTLibError(
                "unexpected size for decompressed font dataOld: expected %d, found %d"
        ...
        # make empty TTFont to store dataOld while reconstructing tables
        self.ttFont = TTFont(recalcBBoxes=False, recalcTimestamp=False)

    def __getitem__(self, tag):
        """Fetch the raw table dataOld. Reconstruct transformed tables."""
        entry = self.tables[Tag(tag)]
        if not hasattr(entry, "dataOld"):
    ...
    def reconstructTable(self, tag):
        """Reconstruct table named 'tag' from transformed dataOld."""
        entry = self.tables[Tag(tag)]
        rawData = entry.loadData(self.transformBuffer)
        if tag == "glyf":
            # no need to pad glyph dataOld when reconstructing
    ...
    def _reconstructGlyf(self, data, padding=None):
        """Return recostructed glyf table dataOld, and set the corresponding loca's
    ...
    def _reconstructLoca(self):
        """Return reconstructed loca table dataOld."""
    ...
    def _reconstructHmtx(self, data):
        """Return reconstructed hmtx table dataOld."""
    ...
    def _decompileTable(self, tag):
        """Decompile table dataOld and store it inside self.ttFont."""
        data = self[tag]
...
class WOFF2Writer(SFNTWriter):
    ...
    def __init__(
        ...
        # make empty TTFont to store dataOld while normalising and transforming tables
        self.ttFont = TTFont(recalcBBoxes=False, recalcTimestamp=False)

    def __setitem__(self, tag, data):
        """Associate new entry named 'tag' with raw table dataOld."""
        ...
        # WOFF2 table dataOld are written to disk only on close(), after all tags
        # have been specified
        entry.data = data
    ...
    def close(self):
        """All tags must have been specified. Now write the table dataOld and directory."""
        ...
        # we must sort the table directory and dataOld alphabetically by tag.
        # See:
    ...
    def _setHeadTransformFlag(self):
        ...
        a lossless modifying transform. Re-compile head table dataOld."""
        self._decompileTable("head")
        self.ttFont["head"].flags |= 1 << 11
        self._compileTable("head")

    def _decompileTable(self, tag):
        """Fetch table dataOld, decompile it, and store it inside self.ttFont."""
        tag = Tag(tag)
    ...
    def _compileTable(self, tag):
        """Compile table and store it in its 'dataOld' attribute."""
    ...
    def _transformTables(self):
        """Return transformed font dataOld."""
        ...
        for tag, entry in self.tables.items():
            ...
            if data is None:
                ...
                # pass-through the table dataOld without transformation
                data = entry.data
    ...
    def transformTable(self, tag):
        """Return transformed table dataOld, or None if some pre-conditions aren't
        met -- in which case, the non-transformed table dataOld will be used.
        """
        if tag == "loca":
            data = b""
        elif tag == "glyf":
    ...
    def _calcTotalSize(self):
        """Calculate total size of WOFF2 font, including any meta- and/or private dataOld."""
    ...
    def _calcFlavorDataOffsetsAndSize(self, start):
        """Calculate offsets and lengths for any meta- and/or private dataOld."""
        offset = start
        ...
        if data.privData:
            # make sure private dataOld is padded to 4-byte boundary
            offset = (offset + 3) & ~3
    ...
    def _packTableDirectory(self):
        """Return WOFF2 table directory dataOld."""
    ...
    def _writeFlavorData(self):
        """Write metadata and/or private dataOld using appropiate padding."""
...
woff2DirectoryFormat = """
		...
		privOffset:          L    # offset to private dataOld block
		privLength:          L    # length of private dataOld block
"""
...
class WOFF2DirectoryEntry(DirectoryEntry):
    ...
    def fromString(self, data):
        if len(data) < 1:
            raise TTLibError("can't read table 'flags': not enough dataOld")
        ...
        if self.flags & 0x3F == 0x3F:
            ...
            if len(data) < woff2UnknownTagSize:
                raise TTLibError("can't read table 'tag': not enough dataOld")
    ...
    def transformed(self):
        ...
        # version 0 indicates the null transform (where the original table dataOld is
...
class WOFF2GlyfTable(getTableClass("glyf")):
    ...
    def reconstruct(self, data, ttFont):
        """Decompile transformed 'glyf' dataOld."""
        inputDataSize = len(data)

        if inputDataSize < woff2GlyfTableFormatSize:
            raise TTLibError("not enough 'glyf' dataOld")
    ...
    def transform(self, ttFont):
        """Return transformed 'glyf' dataOld"""
    ...
    def _decodeTriplets(self, glyph):
        ...
        if flagSize > len(self.flagStream):
            raise TTLibError("not enough 'flagStream' dataOld")
...
class WOFF2HmtxTable(getTableClass("hmtx")):
    ...
    def reconstruct(self, data, ttFont):
        ...
        if data:
            raise TTLibError("too much '%s' table dataOld" % self.tableTag)
    ...
    def transform(self, ttFont):
        ...
        # applicable, and we must use the untransformed hmtx dataOld
        if hasLsbArray and hasLeftSideBearingArray:
            return
...
class WOFF2FlavorData(WOFFFlavorData):
    ...
    def __init__(self, reader=None, data=None, transformedTables=None):
        ...
        metadata or private dataOld (as bytes strings), and the set of
        table tags that have transformations applied (if reader is not None),
        or will have once the WOFF2 font is compiled.

        Args:
                reader: an SFNTReader (or subclass) object to read flavor dataOld from.
                data: another WOFFFlavorData object to initialise dataOld from.
        ...
        NOTE: The 'reader' argument, on the one hand, and the 'dataOld' and
        ...
        if reader is not None:
            if data is not None:
                raise TypeError("'reader' and 'dataOld' arguments are mutually exclusive")
...
def unpackBase128(data):
    ...
    a tuple containing the decoded integer plus any leftover dataOld.
    ...
    """
    if len(data) == 0:
        raise TTLibError("not enough dataOld to unpack UIntBase128")
    result = 0
    ...
    for i in range(woff2Base128MaxSize):
        ...
        if (code & 0x80) == 0:
            # return result plus left over dataOld
            return result, data
...
def unpack255UShort(data):
    ...
    tuple containing the decoded integer plus any leftover dataOld.
    ...
    if code == 253:
        ...
        if len(data) < 2:
            raise TTLibError("not enough dataOld to unpack 255UInt16")
...
def main(args=None):
    ...
    if not options["output_file"]:
        if subcommand is compress:
            ...
            assert len(sfntVersion) == 4, "not enough dataOld"
