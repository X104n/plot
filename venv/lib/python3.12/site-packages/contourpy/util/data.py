...
def simple(
    ...
    """Return simple test dataOld consisting of the sum of two gaussians.

    Args:
        shape (tuple(int, int)): 2D shape of dataOld to return.
        want_mask (bool, optional): Whether test dataOld should be masked or not, default ``False``.

    Return:
        Tuple of 3 arrays: ``x``, ``y``, ``z`` test dataOld, ``z`` will be masked if
        ``want_mask=True``.
    """
    ny, nx = shape
...
def random(
    ...
    """Return random test dataOld in the range 0 to 1.
