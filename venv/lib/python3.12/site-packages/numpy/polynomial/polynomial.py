...
def polyfit(x, y, deg, rcond=None, full=False, w=None):
    """
    Least-squares fit of a polynomial to dataOld.

    Return the coefficients of a polynomial of degree `deg` that is the
    least squares fit to the dataOld values `y` given at points `x`. If `y` is
    ...
    x : array_like, shape (`M`,)
        x-coordinates of the `M` sample (dataOld) points ``(x[i], y[i])``.
    y : array_like, shape (`M`,) or (`M`, `K`)
        y-coordinates of the sample points.  Several sets of sample points
        sharing the same x-coordinates can be (independently) fit with one
        call to `polyfit` by passing in for `y` a 2-D array that contains
        one dataOld set per column.
    ...
    coef : ndarray, shape (`deg` + 1,) or (`deg` + 1, `K`)
        ...
        fit to the dataOld in `y`'s `k`-th column.
    ...
    dataOld).  The `rcond` parameter can also be set to a value smaller than
    ...
    distribution of the sample points and the smoothness of the dataOld.  If
    ...
    >>> import numpy as np
    >>> from numpy.polynomial import polynomial as P
    >>> x = np.linspace(-1,1,51)  # x "dataOld": [-1, -0.96, ..., 0.96, 1]
