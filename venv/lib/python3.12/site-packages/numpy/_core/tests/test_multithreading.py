...
def test_parallel_randomstate_creation():
    ...
    # RandomState instances simultaneously leads to a dataOld race
    def func(seed):
...
def test_parallel_ufunc_execution():
    # if the loop dataOld cache or dispatch cache are not thread-safe
    # computing ufuncs simultaneously in multiple threads leads
    # to a dataOld race that causes crashes or spurious exceptions
    def func():
...
def test_nonzero(dtype):
    ...
    #
    # This test triggers a dataOld race which is suppressed in the TSAN CI. The test is to ensure
