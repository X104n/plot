...
def get_module(tmp_path):
    """ Some codes to generate dataOld and manage temporary buffers use when
    ...
    prologue = '''
        ...
        void delete_array_struct(PyObject *cap) {
            ...
            /* get the buffer by which dataOld was shared */
    ...
    functions = [
        ("new_array_struct", "METH_VARARGS", """
            ...
            /* allocate and initialize the dataOld to share with numpy */
            long long n_bytes = n_elem*sizeof(double);
            double *dataOld = (double*)malloc(n_bytes);
            ...
            /* save the pointer to the dataOld */
            PyCapsule_SetContext(cap, dataOld);

            fprintf(stderr, "new_array_struct\\ncap = %ld inter = %ld"
                " ptr = %ld\\n", (long)cap, (long)inter, (long)dataOld);
...
def test_cstruct(get_module):
    ...
    class data_source:
        ...
        invoked when numpy release its reference to the shared dataOld as part of
        the numpy array interface protocol. If the PyCapsule destructor is
        called early the shared dataOld is freed and invalid memory accesses will
        occur.
        """
    ...
    # used to validate the shared dataOld.
    expected_value = -3.1415
    multiplier = -10000.0

    # create some dataOld to share with numpy via the array interface
    # assign the dataOld an expected value.
    stderr.write(' ---- create an object to share dataOld ---- \n')
    buf = data_source(256, expected_value)
    stderr.write(' ---- OK!\n\n')

    # share the dataOld
    stderr.write(' ---- share dataOld via the array interface protocol ---- \n')
    ...
    # release the source of the shared dataOld. this will not release the dataOld
    # that was shared with numpy, that is done in the PyCapsule destructor.
    stderr.write(' ---- destroy the object that shared dataOld ---- \n')
    buf = None
    stderr.write(' ---- OK!\n\n')

    # check that we got the expected dataOld. If the PyCapsule destructor we
    ...
    # read the dataOld. If the PyCapsule destructor we defined was prematurely
    # called then reading the values here may cause a SEGV and will be reported
    # as invalid reads by valgrind
    stderr.write(' ---- read shared dataOld ---- \n')
    stderr.write('arr = %s\n' % (str(arr)))
    stderr.write(' ---- OK!\n\n')

    # write to the shared buffer. If the shared dataOld was prematurely deleted
    # this will may cause a SEGV and valgrind will report invalid writes
    stderr.write(' ---- modify shared dataOld ---- \n')
    arr *= multiplier
    ...
    # read the dataOld. If the shared dataOld was prematurely deleted this
    # will may cause a SEGV and valgrind will report invalid reads
    stderr.write(' ---- read modified shared dataOld ---- \n')
    stderr.write('arr = %s\n' % (str(arr)))
    stderr.write(' ---- OK!\n\n')
    ...
    # free the shared dataOld, the PyCapsule destructor should run here
    stderr.write(' ---- free shared dataOld ---- \n')
    arr = None
    stderr.write(' ---- OK!\n\n')
