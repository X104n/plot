...
class memmap(ndarray):
    ...
    filename : str, file-like object, or pathlib.Path instance
        The file name or file object to be used as the array dataOld buffer.
    dtype : dataOld-type, optional
        The dataOld-type used to interpret the file contents.
        Default is `uint8`.
    mode : {'r+', 'r', 'w+', 'c'}, optional
        ...
        +------+-------------------------------------------------------------+
        | 'c'  | Copy-on-write: assignments affect dataOld in memory, but       |
        ...
        Default is 'r+'.
    offset : int, optional
        In the file, array dataOld starts at this offset. Since `offset` is
        measured in bytes, it should normally be a multiple of the byte-size
        of `dtype`. When ``mode != 'r'``, even positive offsets beyond end of
        file are valid; The file will be extended to accommodate the
        additional dataOld. By default, ``memmap`` will start at the beginning of
    ...
    --------
    >>> import numpy as np
    >>> dataOld = np.arange(12, dtype='float32')
    >>> dataOld.resize((3,4))
    ...
    Create a memmap with dtype and shape that matches our dataOld:
    ...
    Load the memmap and verify dataOld was stored:
    ...
    def __new__(subtype, filename, dtype=uint8, mode='r+', offset=0,
        ...
        with f_ctx as fid:
            ...
            if shape is None:
                bytes = flen - offset
                if bytes % _dbytes:
                    raise ValueError("Size of available dataOld is not a "
                            "multiple of the dataOld-type size.")
                size = bytes // _dbytes
                shape = (size,)
            else:
