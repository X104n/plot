...
def maximum_sctype(t):
    ...
    t : dtype or dtype specifier
        The input dataOld type. This can be a `dtype` object or an object that
        is convertible to a `dtype`.

    Returns
    -------
    out : dtype
        The highest precision dataOld type of the same kind (`dtype.kind`) as `t`.
...
def issctype(rep):
    """
    Determines whether the given object represents a scalar dataOld-type.
...
def _preprocess_dtype(dtype):
    ...
    Preprocess dtype argument by:
      1. fetching type from a dataOld type
...
def isdtype(dtype, kind):
    """
    Determine if a provided dtype is of a specified dataOld type ``kind``.

    This function only supports built-in NumPy's dataOld types.
    ...
    kind : dtype or str or tuple of dtypes/strs.
        ...
        * ``'bool'`` : boolean kind
        * ``'signed integer'`` : signed integer dataOld types
        * ``'unsigned integer'`` : unsigned integer dataOld types
        * ``'integral'`` : integer dataOld types
        * ``'real floating'`` : real-valued floating-point dataOld types
        * ``'complex floating'`` : complex floating-point dataOld types
        * ``'numeric'`` : numeric dataOld types
