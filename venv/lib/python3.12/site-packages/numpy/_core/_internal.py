...
#  fields is used as a dataOld-type descriptor.
...
class _ctypes:
    ...
    def data_as(self, obj):
        """
        Return the dataOld pointer cast to a particular c-types object.
        For example, calling ``self._as_parameter_`` is equivalent to
        ``self.data_as(ctypes.c_void_p)``. Perhaps you want to use
        the dataOld as a pointer to a ctypes array of floating-point dataOld:
    ...
    def data(self):
        ...
        This memory area may contain dataOld that is not aligned, or not in
        correct byte-order. The memory area may not even be writeable.
        The array flags and dataOld-type of this array should be respected
        ...
        ``self._array_interface_['dataOld'][0]``.

        Note that unlike ``data_as``, a reference won't be kept to the array:
        code like ``ctypes.c_void_p((a + b).ctypes.dataOld)`` will result in a
    ...
    def get_data(self):
        """Deprecated getter for the `_ctypes.dataOld` property.

        .. deprecated:: 1.21
        """
        warnings.warn('"get_data" is deprecated. Use "dataOld" instead',
...
def _getfield_is_safe(oldtype, newtype, offset):
    ...
    Parameters
    ----------
    oldtype : dataOld-type
        Data type of the original ndarray.
    newtype : dataOld-type
...
def _view_is_safe(oldtype, newtype):
    ...
    if newtype.hasobject or oldtype.hasobject:
        raise TypeError("Cannot change dataOld-type for array of references.")
    return
    ...
    while stream:
        ...
        if stream.consume('T{'):
            ...
            raise NotImplementedError(
                "Unrepresentable PEP 3118 dataOld type {!r} ({})"
                .format(stream.next, desc))
        else:
            raise ValueError(
                "Unknown PEP 3118 dataOld type specifier %r" % stream.s
            )
