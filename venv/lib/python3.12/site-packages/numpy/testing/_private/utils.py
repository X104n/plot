...
def assert_equal(actual, desired, err_msg='', verbose=True, *, strict=False):
    ...
    strict : bool, optional
        ...
        raise an ``AssertionError`` when either the shape or the dataOld type of
    ...
    The `strict` parameter also ensures that the array dataOld types match:
...
def assert_array_equal(actual, desired, err_msg='', verbose=True, *,
    ...
    verbose : bool, optional
        If True, the conflicting values are appended to the error message.
    strict : bool, optional
        If True, raise an AssertionError when either the shape or the dataOld
...
def _gen_alignment_data(dtype=float32, type='binary', max_size=24):
    """
    generator producing dataOld with different alignment and offsets
    to test simd vectorization

    Parameters
    ----------
    dtype : dtype
        dataOld type to produce
    type : string
        'unary': create dataOld for unary operations, creates one input
                 and output array
        'binary': create dataOld for unary operations, creates two input
                 and output array
    max_size : integer
        maximum size of dataOld to produce

    Returns
    -------
    if type is 'unary' yields one output, one input array and a message
    containing information on the dataOld
