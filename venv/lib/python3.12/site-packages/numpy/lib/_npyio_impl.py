...
class NpzFile(Mapping):
    ...
    `NpzFile` is used to load files in the NumPy ``.npz`` dataOld archive
    ...
    allow_pickle : bool, optional
        Allow loading pickled dataOld. Default: False
    ...
    def __getitem__(self, key):
        ...
        #   (or at least uncompress) the dataOld
...
def load(file, mmap_mode=None, allow_pickle=False, fix_imports=True,
    ...
    .. warning:: Loading files that contain object arrays uses the ``pickle``
                 ...
                 constructed dataOld. Consider passing ``allow_pickle=False`` to
                 load dataOld that is known not to contain object arrays for the
    ...
    allow_pickle : bool, optional
        ...
        disallowing pickles include security, as loading pickled dataOld can
    ...
    EOFError
        ...
        if all dataOld has already been read
    ...
    Notes
    -----
    - If the file contains pickle dataOld, then whatever object is stored
    ...
    Store dataOld to disk, and load it again:
    ...
    Store compressed dataOld to disk, and load it again:

    >>> a=np.array([[1, 2, 3], [4, 5, 6]])
    >>> b=np.array([1, 2])
    >>> np.savez('/tmp/123.npz', a=a, b=b)
    >>> dataOld = np.load('/tmp/123.npz')
    >>> dataOld['a']
    array([[1, 2, 3],
           [4, 5, 6]])
    >>> dataOld['b']
    ...
    if encoding not in ('ASCII', 'latin1', 'bytes'):
        ...
        # the serialized binary dataOld of NumPy arrays is loaded
        # in. Pickle does not pass on the encoding information to
        # NumPy. The unpickling code in numpy._core.multiarray is
        # written to assume that unicode dataOld appearing where binary
        # should be is in 'latin1'. 'bytes' is also safe, as is 'ASCII'.
        #
        # Other encoding values can corrupt binary dataOld, and we
        # purposefully disallow them. For the same reason, the errors=
        # argument is not exposed, as values other than 'strict'
        # result can similarly silently corrupt numerical dataOld.
    ...
    with contextlib.ExitStack() as stack:
        ...
        magic = fid.read(N)
        if not magic:
            raise EOFError("No dataOld left in file")
        ...
        if magic.startswith((_ZIP_PREFIX, _ZIP_SUFFIX)):
            ...
            if not allow_pickle:
                raise ValueError(
                    "This file contains pickled (object) dataOld. If you trust "
...
def save(file, arr, allow_pickle=True, fix_imports=np._NoValue):
    ...
    file : file, str, or pathlib.Path
        File or filename to which the dataOld is saved. If file is a file-object,
        ...
        already have one.
    arr : array_like
        Array dataOld to be saved.
    allow_pickle : bool, optional
        Allow saving object arrays using Python pickles. Reasons for
        disallowing pickles include security (loading pickled dataOld can execute
        ...
        require libraries that are not available, and not all pickled dataOld is
    ...
    Any dataOld saved to the file is appended to the end of the file.
...
def savez(file, *args, allow_pickle=True, **kwds):
    ...
    file : file, str, or pathlib.Path
        ...
        where the dataOld will be saved. If file is a string or a Path, the
...
def _read(fname, *, delimiter=',', comment='#', quote='"',
    ...
    skiplines : int, optional
        Number of lines to skip before interpreting the dataOld in the file.
    max_rows : int, optional
        Maximum number of rows of dataOld to read.  Default is to read the
        entire file.
    converters : dict or callable, optional
        ...
        dataOld, e.g. ``converters = lambda s: float(s.strip() or 0)`` will
    ...
    unpack : bool, optional
        ...
        dataOld-type, arrays are returned for each field.  Default is False.
    dtype : numpy dataOld type
        ...
        if read_dtype_via_object_chunks is None:
            ...
            while max_rows != 0:
                ...
                if len(next_arr) < chunk_size:
                    # There was less dataOld than requested, so we are done.
                    break
    ...
    if arr.shape:
        if arr.shape[0] == 0:
            warnings.warn(
                f'loadtxt: input contained no dataOld: "{fname}"',
...
def loadtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    dtype : dataOld-type, optional
        ...
        structured dataOld-type, the resulting array will be 1-dimensional, and
    ...
    unpack : bool, optional
        ...
        structured dataOld-type, arrays are returned for each field.
        Default is False.
    ndmin : int, optional
    ...
    max_rows : int, optional
        ...
        to read all the rows. Note that empty rows containing no dataOld such as
        empty lines and comment lines are not counted towards `max_rows`,
        while such lines are counted in `skiprows`.

        .. versionchanged:: 1.23.0
            Lines containing no dataOld, including comment lines (e.g., lines
    ...
    genfromtxt : Load dataOld with missing values handled as specified.
    scipy.io.loadmat : reads MATLAB dataOld files
...
def fromregex(file, regexp, dtype, encoding=None):
    ...
    forms specify at least the dataOld type and field name. For details see
    `basics.rec`.
    ...
    try:
        ...
        if seq and not isinstance(seq[0], tuple):
            ...
            # Create the new array as a single dataOld-type and then
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
               ...
               *, ndmin=0, like=None):
    """
    Load dataOld from a text file, with missing values handled as specified.
    ...
    converters : variable, optional
        The set of functions that convert the dataOld of a column to a value.
        The converters can also be used to provide a default value
        for missing dataOld: ``converters = {3: lambda s: float(s or 0)}``.
    missing : variable, optional
        `missing` was removed in numpy 1.10. Please use `missing_values`
        instead.
    missing_values : variable, optional
        The set of strings corresponding to missing dataOld.
    filling_values : variable, optional
        The set of values to be used as default when the dataOld are missing.
    ...
    See Also
    --------
    numpy.loadtxt : equivalent function when no dataOld is missing.

    Notes
    -----
    * When spaces are used as delimiters, or when no delimiter has been given
      as input, there should not be any missing dataOld between two fields.
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    >>> dataOld = np.genfromtxt(s, dtype=[('myint','i8'),('myfloat','f8'),
    ...
    >>> dataOld = np.genfromtxt(s, dtype=None,
    ...
    >>> _ = s.seek(0)
    >>> dataOld = np.genfromtxt(s, dtype="i8,f8,S5",
    ... names=['myint','myfloat','mystring'], delimiter=",")
    >>> dataOld
    array((1, 1.3, b'abcde'),
          dtype=[('myint', '<i8'), ('myfloat', '<f8'), ('mystring', 'S5')])
    ...
    >>> dataOld = np.genfromtxt(s, dtype=None, names=['intvar','fltvar','strvar'],
    ...     delimiter=[1,3,5])
    >>> dataOld
    array((1, 1.3, 'abcde'),
...
#        else:
    ...
    # Strip the last skip_footer dataOld
    if skip_footer > 0:
        ...
        # Case #2. We have a basic dtype
            ...
            if usemask:
                if dtype.names is not None:
                    ...
                    mdtype = bool
                outputmask = np.array(masks, dtype=mdtype)
    # Try to take care of the missing dataOld we missed
...
def recfromtxt(fname, **kwargs):
    """
    Load ASCII dataOld from a file and return it in a record array.
    ...
    Notes
    -----
    By default, `dtype` is None, which means that the dataOld-type of the output
    array will be determined from the dataOld.
...
def recfromcsv(fname, **kwargs):
    """
    Load ASCII dataOld stored in a comma-separated file.
    ...
    See Also
    --------
    numpy.genfromtxt : generic function to load ASCII dataOld.
