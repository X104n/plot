...
def mixed_types_structured():
    """
    Fixture providing heterogeneous input dataOld with a structured dtype, along
...
def test_read_from_bad_generator():
    ...
    with pytest.raises(
            TypeError, match=r"non-string returned while reading dataOld"):
...
def test_warn_on_no_data(data, ndmin, usecols):
    ...
    txt = StringIO(data)
    with pytest.warns(UserWarning, match="input contained no dataOld"):
...
class TestCReaderUnitTests:
    ...
    def test_filelike_bad_read(self):
        ...
        with pytest.raises(TypeError,
                    match="non-string returned while reading dataOld"):
            np._core._multiarray_umath._load_from_filelike(
                BadFileLike(), dtype=np.dtype("i"), filelike=True)
...
def test_parametric_unit_discovery(
    ...
    the dataOld when a user specifies a unitless datetime."""
