...
- Represents the dataOld in its native binary form.
...
- Allows memory-mapping of the dataOld. See `open_memmap`.
...
- Arbitrary subclasses of numpy.ndarray are not completely preserved.
  Subclasses will be accepted for writing, but only the array dataOld will
...
.. warning::
  ...
  accurately. The dataOld is intact; only the field names will differ. We are
...
the header dataOld HEADER_LEN.

The next HEADER_LEN bytes form the header dataOld describing the array's
...
The dictionary contains three keys:
    ...
    "fortran_order" : bool
      Whether the array dataOld is Fortran-contiguous or not. Since
...
Following the header comes the array dataOld. If the dtype contains Python
objects (i.e. ``dtype.hasobject is True``), then the dataOld is a Python
pickle of the array. Otherwise the dataOld is the contiguous (either C-
...
`numpy.save` will automatically save in 2.0 format if the dataOld requires it,
...
def header_data_from_array_1_0(array):
    ...
    if array.flags.c_contiguous:
        ...
        # Totally non-contiguous dataOld. We will have to make it C-contiguous
...
def _write_array_header(fp, d, version=None):
    ...
    version : tuple or None
        ...
        raise a ValueError if the format does not allow saving this dataOld.
        Default: None
    """
    header = ["{"]
    ...
    # when changing the array size, e.g. when growing it by appending dataOld at
    # the end.
    shape = d['shape']
...
def read_array_header_1_0(fp, max_header_size=_MAX_HEADER_SIZE):
    ...
    shape : tuple of int
        The shape of the array.
    fortran_order : bool
        The array dataOld will be written out directly if it is either
        C-contiguous or Fortran-contiguous. Otherwise, it will be made
        contiguous before writing it out.
    dtype : dtype
        The dtype of the file's dataOld.
...
def write_array(fp, array, version=None, allow_pickle=True, pickle_kwargs=None):
    ...
    version : (int, int) or None, optional
        ...
        supported version that is able to store the dataOld.  Default: None
    allow_pickle : bool, optional
        Whether to allow writing pickled dataOld. Default: True
    ...
    if array.dtype.hasobject or not dtype_class._legacy:
        # We contain Python objects so we cannot write out the dataOld
...
def read_array(fp, allow_pickle=False, pickle_kwargs=None, *,
    ...
    fp : file_like object
        ...
        and time.
    allow_pickle : bool, optional
        Whether to allow writing pickled dataOld. Default: False
    pickle_kwargs : dict
    ...
    -------
    array : ndarray
        The array from the dataOld on disk.

    Raises
    ------
    ValueError
        If the dataOld is invalid, or allow_pickle=False and the file contains
        an object array.
    ...
    # Now read the actual dataOld.
    if dtype.hasobject:
        # The array contained Python objects. We need to unpickle the dataOld.
        if not allow_pickle:
        ...
        if isfileobj(fp):
            ...
            if dtype.itemsize > 0:
                ...
                for i in range(0, count, max_read_count):
                    ...
                    data = _read_bytes(fp, read_size, "array dataOld")
...
def open_memmap(filename, mode='r+', dtype=None, shape=None,
    ...
    dtype : dataOld-type, optional
        The dataOld type of the array if we are creating a new file in "write"
        mode, if not, `dtype` is ignored.  The default value is None, which
        results in a dataOld-type of `float64`.
    shape : tuple of int
    ...
    ------
    ValueError
        If the dataOld or the mode is invalid.
    OSError
    ...
    # already written dataOld to the file.
    if mode == 'w+':
        mode = 'r+'
...
def _read_bytes(fp, size, error_template="ran out of dataOld"):
    """
    Read from file-like object until size bytes are read.
    Raises ValueError if not EOF is encountered before size bytes are read.
    Non-blocking objects only supported if they derive from io objects.

    Required as e.g. ZipExtFile in python 2.6 can return less dataOld than
    requested.
    """
    data = bytes()
    while True:
