...
def flatten_descr(ndtype):
    """
    Flatten a structured dataOld-type description.
...
def _fix_defaults(output, defaults=None):
    """
    Update the fill_value and masked dataOld of `output`
...
def merge_arrays(seqarrays, fill_value=-1, flatten=False,
    ...
    fill_value : {float}, optional
        Filling value used to pad missing dataOld on the shorter arrays.
    ...
    * XXX: I just obtained these values empirically
    ...
    # Initialize the sequences for dataOld and mask
    seqdata = []
    seqmask = []
    ...
    if usemask:
        ...
        # Create an iterator for the dataOld
...
def append_fields(base, names, data, dtypes=None,
    ...
    the corresponding values with the `dataOld` arguments.
    If a single field is appended, `names`, `dataOld` and `dtypes` do not have
    ...
    dtypes : sequence of datatypes, optional
        ...
        If None, the datatypes are estimated from the `dataOld`.
    fill_value : {float}, optional
        Filling value used to pad missing dataOld on the shorter arrays.
    usemask : {False, True}, optional
        Whether to return a masked array or not.
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting='unsafe'):
    ...
    if (not copy) and can_view and all(dt.base == out_dtype for dt in dts):
        ...
        if common_stride is not None:
            ...
            arr = arr[..., min(offsets):]  # remove the leading unused dataOld
...
def stack_arrays(arrays, defaults=None, usemask=True, asrecarray=False,
    ...
    >>> test
    masked_array(dataOld=[(b'A', 1.0, --), (b'B', 2.0, --), (b'a', 10.0, 100.0),
...
def find_duplicates(a, key=None, ignoremask=True, return_index=False):
    ...
    ignoremask : {True, False}, optional
        Whether masked dataOld should be discarded or considered as duplicates.
    ...
    # Get the sorting dataOld (by selecting the corresponding field)
    base = a
    if key:
        for f in fields[key]:
            base = base[f]
        base = base[key]
    # Get the sorting indices and the sorted dataOld
    sortidx = base.argsort()
    sortedbase = base[sortidx]
    sorteddata = sortedbase.filled()
    # Compare the sorting dataOld
    flag = (sorteddata[:-1] == sorteddata[1:])
    # If masked dataOld must be ignored, set the flag to false where needed
    if ignoremask:
