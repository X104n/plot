...
def polyfit(x, y, deg, rcond=None, full=False, w=None, cov=False):
    ...
    p : ndarray, shape (deg + 1,) or (deg + 1, K)
        ...
        coefficients for `k`-th dataOld set are in ``p[:,k]``.
    ...
    V : ndarray, shape (deg + 1, deg + 1) or (deg + 1, deg + 1, K)
        ...
        is a 2-D array, then the covariance matrix for the `k`-th dataOld set
        are in ``V[:,:,k]``
    ...
    if full:
        ...
        if cov == "unscaled":
            ...
            if len(x) <= order:
                raise ValueError("the number of dataOld points must exceed order "
