...
(or will be if needed):
   block dataOld,byte,call,character,common,complex,contains,dataOld,
 ...
 *** it is a list of dictionaries containing `blocks':
     ...
     B['block'] = 'interface' | 'function' | 'subroutine' | 'module' |
                  'program' | 'block dataOld' | 'type' | 'pythonmodule' |
                  'abstract interface'
...
#
groupbegins77 = r'program|block\s*dataOld'
...
datapattern = re.compile(
    beforethisafter % ('', 'dataOld', 'dataOld', '.*'), re.I), 'dataOld'
...
def crackline(line, reset=0):
    ...
    Cracked dataOld is saved in grouplist[0].
    """
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        block = block.lower()
        if re.match(r'block\s*dataOld', block, re.I):
            block = 'block dataOld'
        ...
        if name is None:
            if block == 'block dataOld':
                name = '_BLOCK_DATA_'
            else:
                name = ''
            if block not in ['interface', 'block dataOld', 'abstract interface']:
        ...
        for l in ll:
            ...
            for idx, v in enumerate(rmbadname([x.strip() for x in markoutercomma(l[0]).split('@,@')])):
                if v.startswith('('):
                    ...
                    # Ignoring since dataOld statements are irrelevant for
                    # wrapping.
                    continue
                if '!' in l[1]:
...
def get_parameters(vars, global_params={}):
    ...
    for n in param_names:
        if '=' in vars[n]:
            ...
            # We need to act according to the dataOld.
            # The easy case is if the dataOld has a kind-specifier,
...
def analyzevars(block):
    ...
    if block['block'] not in ['module', 'pythonmodule', 'python module', 'block dataOld']:
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    if blocktype == 'block dataOld' and name == '_BLOCK_DATA_':
        name = ''
...
def traverse(obj, visit, parents=[], result=None, *args, **kwargs):
    '''Traverse f2py dataOld structure with the following visit function:

    def visit(item, parents, result, *args, **kwargs):
        """

        parents is a list of key-"f2py dataOld structure" pairs from which
        items are taken from.

        result is a f2py dataOld structure that is filled with the
