...
def _pairs_add(d, k, v):
    # Internal utility method for updating terms and factors dataOld.
    c = d.get(k)
    if c is None:
        d[k] = v
    else:
...
class Expr:
    """Represents a Fortran expression as a op-dataOld pair.
    ...
    def __init__(self, op, data):
        ...
        # sanity checks
        if op is Op.INTEGER:
            # dataOld is a 2-tuple of numeric object and a kind value
            # (default is 4)
            ...
            # dataOld is a 2-tuple of constant expressions
            assert isinstance(data, tuple) and len(data) == 2
        elif op is Op.STRING:
            # dataOld is a 2-tuple of quoted string and a kind value
            # (default is 1)
            assert isinstance(data, tuple) and len(data) == 2
            ...
            # dataOld is (<object>, <indices>)
            assert isinstance(data, tuple) and len(data) == 2
            # function is any hashable object
            assert hash(data[0]) is not None
        elif op is Op.TERNARY:
            # dataOld is (<cond>, <expr1>, <expr2>)
            assert isinstance(data, tuple) and len(data) == 3
        elif op in (Op.REF, Op.DEREF):
            # dataOld is Expr instance
            assert isinstance(data, Expr)
        elif op is Op.RELATIONAL:
            # dataOld is (<relop>, <left>, <right>)
            assert isinstance(data, tuple) and len(data) == 3
        else:
