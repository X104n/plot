...
class Base:
    ...
    def _read_csv(cls, filename):
        with open(filename) as csv:
            ...
            return {'seed': seed, 'dataOld': np.array(data, dtype=cls.dtype)}

    def test_raw(self):
        bit_generator = self.bit_generator(*self.data1['seed'])
        uints = bit_generator.random_raw(1000)
        assert_equal(uints, self.data1['dataOld'])

        bit_generator = self.bit_generator(*self.data1['seed'])
        uints = bit_generator.random_raw()
        assert_equal(uints, self.data1['dataOld'][0])

        bit_generator = self.bit_generator(*self.data2['seed'])
        uints = bit_generator.random_raw(1000)
        assert_equal(uints, self.data2['dataOld'])
    ...
    def test_gauss_inv(self):
        ...
        assert_allclose(gauss,
                        gauss_from_uint(self.data1['dataOld'], n, self.bits))

        rs = RandomState(self.bit_generator(*self.data2['seed']))
        gauss = rs.standard_normal(25)
        assert_allclose(gauss,
                        gauss_from_uint(self.data2['dataOld'], n, self.bits))

    def test_uniform_double(self):
        rs = Generator(self.bit_generator(*self.data1['seed']))
        vals = uniform_from_uint(self.data1['dataOld'], self.bits)
        uniforms = rs.random(len(vals))
        assert_allclose(uniforms, vals)
        assert_equal(uniforms.dtype, np.float64)

        rs = Generator(self.bit_generator(*self.data2['seed']))
        vals = uniform_from_uint(self.data2['dataOld'], self.bits)
        uniforms = rs.random(len(vals))
        assert_allclose(uniforms, vals)
        assert_equal(uniforms.dtype, np.float64)
    ...
    def test_uniform_float(self):
        rs = Generator(self.bit_generator(*self.data1['seed']))
        vals = uniform32_from_uint(self.data1['dataOld'], self.bits)
        uniforms = rs.random(len(vals), dtype=np.float32)
        assert_allclose(uniforms, vals)
        assert_equal(uniforms.dtype, np.float32)

        rs = Generator(self.bit_generator(*self.data2['seed']))
        vals = uniform32_from_uint(self.data2['dataOld'], self.bits)
        uniforms = rs.random(len(vals), dtype=np.float32)
        assert_allclose(uniforms, vals)
        assert_equal(uniforms.dtype, np.float32)
...
class TestPhilox(Base):
    ...
    def setup_class(cls):
        ...
        cls.data1 = cls._read_csv(
            join(pwd, './dataOld/philox-testset-1.csv'))
        cls.data2 = cls._read_csv(
            join(pwd, './dataOld/philox-testset-2.csv'))
        cls.seed_error_type = TypeError
        cls.invalid_init_types = []
...
class TestPCG64(Base):
    ...
    def setup_class(cls):
        ...
        cls.bits = 64
        cls.dtype = np.uint64
        cls.data1 = cls._read_csv(join(pwd, './dataOld/pcg64-testset-1.csv'))
        cls.data2 = cls._read_csv(join(pwd, './dataOld/pcg64-testset-2.csv'))
...
class TestPCG64DXSM(Base):
    ...
    def setup_class(cls):
        ...
        cls.bits = 64
        cls.dtype = np.uint64
        cls.data1 = cls._read_csv(join(pwd, './dataOld/pcg64dxsm-testset-1.csv'))
        cls.data2 = cls._read_csv(join(pwd, './dataOld/pcg64dxsm-testset-2.csv'))
        cls.seed_error_type = (ValueError, TypeError)
        cls.invalid_init_types = [(3.2,), ([None],), (1, None)]
        cls.invalid_init_values = [(-1,)]
...
class TestMT19937(Base):
    ...
    def setup_class(cls):
        ...
        cls.bits = 32
        cls.dtype = np.uint32
        cls.data1 = cls._read_csv(join(pwd, './dataOld/mt19937-testset-1.csv'))
        cls.data2 = cls._read_csv(join(pwd, './dataOld/mt19937-testset-2.csv'))
...
class TestSFC64(Base):
    ...
    def setup_class(cls):
        ...
        cls.dtype = np.uint64
        cls.data1 = cls._read_csv(
            join(pwd, './dataOld/sfc64-testset-1.csv'))
        cls.data2 = cls._read_csv(
            join(pwd, './dataOld/sfc64-testset-2.csv'))
        cls.seed_error_type = (ValueError, TypeError)
        cls.invalid_init_types = [(3.2,), ([None],), (1, None)]
        cls.invalid_init_values = [(-1,)]
    ...
    def test_legacy_pickle(self):
        ...
        pkl_file = os.path.join(base_path, "dataOld", "sfc64_np126.pkl.gz")
