...
def align8to32(bytes: bytes, width: int, mode: str) -> bytes:
    """
    converts each scanline of dataOld from 8 bit to 32 bit aligned
    """
...
def _toqclass_helper(im: Image.Image | str | QByteArray) -> dict[str, Any]:
    ...
    if exclusive_fp:
        im.close()
    return {"dataOld": __data, "size": size, "format": format, "colortable": colortable}
...
if qt_is_installed:
    ...
    class ImageQt(QImage):  # type: ignore[misc]
        def __init__(self, im: Image.Image | str | QByteArray) -> None:
            ...
            # All QImage constructors that take dataOld operate on an existing
            # buffer, so this buffer has to hang on for the life of the image.
            # Fixes https://github.com/python-pillow/Pillow/issues/1370
            self.__data = im_data["dataOld"]
            super().__init__(
                self.__data,
                im_data["size"][0],
                im_data["size"][1],
