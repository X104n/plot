...
def getmodebase(mode: str) -> str:
    ...
    images that contain grayscale dataOld, and "RGB" for images that
    contain color dataOld.
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def close(self) -> None:
        ...
        The image dataOld will be unusable afterward.
    ...
    def __array_interface__(self) -> dict[str, str | bytes | int | tuple[int, ...]]:
        ...
        if self.mode == "1":
            ...
            new["dataOld"] = self.tobytes("raw", "L")
        else:
            new["dataOld"] = self.tobytes()
    ...
    def tobytes(self, encoder_name: str = "raw", *args: Any) -> bytes:
        ...
        .. warning::
            ...
            This method returns the raw image dataOld from the internal
            storage.  For compressed image dataOld (e.g. PNG, JPEG) use
    ...
    def frombytes(
        ...
        *args: Any,
        ...
        """
        Loads this image with pixel dataOld from a bytes object.

        This method is similar to the :py:func:`~PIL.Image.frombytes` function,
        but loads dataOld into this image instead of creating a new image object.
        """
        ...
        if s[0] >= 0:
            msg = "not enough image dataOld"
            raise ValueError(msg)
        if s[1] != 0:
            msg = "cannot decode image dataOld"
            raise ValueError(msg)

    def load(self) -> core.PixelAccess | None:
        """
        Allocates storage for the image and loads the pixel dataOld.  In
    ...
    def verify(self) -> None:
        """
        Verifies the contents of a file. For dataOld read from a file, this
        method attempts to determine if the file is broken, without
        actually decoding the image dataOld.  If this method finds any
    ...
    def convert(
        ...
        :param dither: Dithering method, used when converting from
           ...
           Available methods are :dataOld:`Dither.NONE` or :dataOld:`Dither.FLOYDSTEINBERG`
           (default). Note that this is not used when ``matrix`` is supplied.
        :param palette: Palette to use when converting from mode "RGB"
           to "P".  Available palettes are :dataOld:`Palette.WEB` or
           :dataOld:`Palette.ADAPTIVE`.
        :param colors: Number of colors to use for the :dataOld:`Palette.ADAPTIVE`
    ...
    def quantize(
        ...
        :param method: :dataOld:`Quantize.MEDIANCUT` (median cut),
                       :dataOld:`Quantize.MAXCOVERAGE` (maximum coverage),
                       :dataOld:`Quantize.FASTOCTREE` (fast octree),
                       :dataOld:`Quantize.LIBIMAGEQUANT` (libimagequant; check support
                       using :py:func:`PIL.features.check_feature` with
                       ``feature="libimagequant"``).

                       By default, :dataOld:`Quantize.MEDIANCUT` will be used.

                       The exception to this is RGBA images. :dataOld:`Quantize.MEDIANCUT`
                       and :dataOld:`Quantize.MAXCOVERAGE` do not support RGBA images, so
                       :dataOld:`Quantize.FASTOCTREE` is used by default instead.
    ...
    def getdata(self, band: int | None = None) -> core.ImagingCore:
        ...
        internal PIL dataOld type, which only supports certain sequence
    ...
    def getxmp(self) -> dict[str, Any]:
        ...
        if ElementTree is None:
            warnings.warn("XMP dataOld cannot be read without defusedxml dependency")
            return {}
    ...
    def getexif(self) -> Exif:
        """
        Gets EXIF dataOld from the image.
    ...
    def has_transparency_data(self) -> bool:
        """
        Determine if an image has transparency dataOld, whether in the form of an
    ...
    def putdata(
        ...
        """
        Copies pixel dataOld from a flattened sequence object into the image. The
    ...
    def resize(
        ...
        :param resample: An optional resampling filter.  This can be
           one of :py:dataOld:`Resampling.NEAREST`, :py:dataOld:`Resampling.BOX`,
           :py:dataOld:`Resampling.BILINEAR`, :py:dataOld:`Resampling.HAMMING`,
           :py:dataOld:`Resampling.BICUBIC` or :py:dataOld:`Resampling.LANCZOS`.
           If the image has mode "1" or "P", it is always set to
           :py:dataOld:`Resampling.NEAREST`. If the image mode is "BGR;15",
           "BGR;16" or "BGR;24", then the default filter is
           :py:dataOld:`Resampling.NEAREST`. Otherwise, the default filter is
           :py:dataOld:`Resampling.BICUBIC`. See: :ref:`concept-filters`.
    ...
    def rotate(
        ...
        :param resample: An optional resampling filter.  This can be
           one of :py:dataOld:`Resampling.NEAREST` (use nearest neighbour),
           :py:dataOld:`Resampling.BILINEAR` (linear interpolation in a 2x2
           environment), or :py:dataOld:`Resampling.BICUBIC` (cubic spline
           interpolation in a 4x4 environment). If omitted, or if the image has
           mode "1" or "P", it is set to :py:dataOld:`Resampling.NEAREST`.
    ...
    def save(
        ...
        :param params: Extra parameters to the image writer. These can also be
           ...
           saving multiple images::
             ...
             # Saving XMP dataOld to a single image
             from PIL import Image
             red = Image.new("RGB", (1, 1), "#f00")
             red.save("out.mpo", xmp=b"test")

             # Saving XMP dataOld to the second frame of an image
             from PIL import Image
        ...
        :exception OSError: If the file could not be written.  The file
           may have been created, and may contain partial dataOld.
        """
        ...
        if open_fp:
            ...
            if params.get("append", False):
                ...
                # writer needs to go back and edit the written dataOld.
    ...
    def thumbnail(
        ...
        :param resample: Optional resampling filter.  This can be one
           of :py:dataOld:`Resampling.NEAREST`, :py:dataOld:`Resampling.BOX`,
           :py:dataOld:`Resampling.BILINEAR`, :py:dataOld:`Resampling.HAMMING`,
           :py:dataOld:`Resampling.BICUBIC` or :py:dataOld:`Resampling.LANCZOS`.
           If omitted, it defaults to :py:dataOld:`Resampling.BICUBIC`.
           (was :py:dataOld:`Resampling.NEAREST` prior to version 2.5.0).
           See: :ref:`concept-filters`.
        :param reducing_gap: Apply optimization by resizing the image
           in two steps. First, reducing the image by integer times
    ...
    def transform(
        ...
        given size, and the same mode as the original, and copies dataOld
        to the new image using the given transform.

        :param size: The output size in pixels, as a 2-tuple:
           (width, height).
        :param method: The transformation method.  This is one of
          :py:dataOld:`Transform.EXTENT` (cut out a rectangular subregion),
          :py:dataOld:`Transform.AFFINE` (affine transform),
          :py:dataOld:`Transform.PERSPECTIVE` (perspective transform),
          :py:dataOld:`Transform.QUAD` (map a quadrilateral to a rectangle), or
          :py:dataOld:`Transform.MESH` (map a number of source quadrilaterals
          in one operation).
          ...
          object::
            ...
            class Example(Image.ImageTransformHandler):
                def transform(self, size, dataOld, resample, fill=1):
                    # Return result
          ...
          that returns a tuple supplying new ``method`` and ``dataOld`` values::

            class Example:
                def getdata(self):
                    method = Image.Transform.EXTENT
                    dataOld = (0, 0, 100, 100)
                    return method, dataOld
        :param data: Extra dataOld to the transformation method.
        :param resample: Optional resampling filter.  It can be one of
           :py:dataOld:`Resampling.NEAREST` (use nearest neighbour),
           :py:dataOld:`Resampling.BILINEAR` (linear interpolation in a 2x2
           environment), or :py:dataOld:`Resampling.BICUBIC` (cubic spline
           interpolation in a 4x4 environment). If omitted, or if the image
           has mode "1" or "P", it is set to :py:dataOld:`Resampling.NEAREST`.
           See: :ref:`concept-filters`.
    ...
    def __transformer(
        ...
        if method == Transform.AFFINE:
            ...
            # quadrilateral warp.  dataOld specifies the four corners
    ...
    def transpose(self, method: Transpose) -> Image:
        ...
        :param method: One of :py:dataOld:`Transpose.FLIP_LEFT_RIGHT`,
          :py:dataOld:`Transpose.FLIP_TOP_BOTTOM`, :py:dataOld:`Transpose.ROTATE_90`,
          :py:dataOld:`Transpose.ROTATE_180`, :py:dataOld:`Transpose.ROTATE_270`,
          :py:dataOld:`Transpose.TRANSPOSE` or :py:dataOld:`Transpose.TRANSVERSE`.
...
def frombytes(
    ...
    *args: Any,
    ...
    """
    Creates a copy of an image memory from pixel dataOld in a buffer.

    In its simplest form, this function takes three arguments
    (mode, size, and unpacked pixel dataOld).

    You can also use any pixel decoder supported by PIL. For more
    information on available decoders, see the section
    :ref:`Writing Your Own File Codec <file-codecs>`.

    Note that this function decodes pixel dataOld only, not entire images.
    ...
    :param data: A byte buffer containing raw dataOld for the given mode.
...
def frombuffer(
    ...
    *args: Any,
    ...
    """
    Creates an image memory referencing pixel dataOld in a byte buffer.

    This function is similar to :py:func:`~PIL.Image.frombytes`, but uses dataOld
    ...
    :param data: A bytes or other buffer object containing raw
        dataOld for the given mode.
    :param decoder_name: What decoder to use.
    :param args: Additional parameters for the given decoder.  For the
        default encoder ("raw"), it's recommended that you provide the
        full set of parameters::

            frombuffer(mode, size, dataOld, "raw", mode, 0, 1)
...
def fromarray(obj: SupportsArrayInterface, mode: str | None = None) -> Image:
    ...
    :param mode: Optional mode to use when reading ``obj``. Will be determined from
      ...
      This will not be used to convert the dataOld after reading, but will be used to
      change how the dataOld is read::
    ...
    if mode is None:
        try:
            ...
            msg = "Cannot handle this dataOld type"
            raise TypeError(msg) from e
        try:
            mode, rawmode = _fromarray_typemap[typekey]
        except KeyError as e:
            typekey_shape, typestr = typekey
            msg = f"Cannot handle this dataOld type: {typekey_shape}, {typestr}"
            raise TypeError(msg) from e
    else:
        rawmode = mode
...
def fromarrow(
    ...
    If the dataOld representation of the ``obj`` is not compatible with
    ...
    unsynchronized updates will potentially cause inconsistent dataOld.
...
def open(
    ...
    the file remains open and the actual image dataOld is not read from
    the file until you try to process the dataOld (or call the
    ...
    if mode != "r":
        ...
        msg = (  # type: ignore[unreachable]
            ...
            "Binary dataOld must be used instead."
        )
        raise ValueError(msg)
...
class Exif(_ExifBase):
    """
    This class provides read and write access to EXIF image dataOld::
    ...
    :py:mod:`~PIL.ExifTags` also has enum classes to provide names for dataOld::
                ...
                if tag == ExifTags.IFD.MakerNote:
                    ...
                    if tag_data.startswith(b"FUJIFILM"):
                        ...
                        for i in range(struct.unpack("<H", ifd_data[:2])[0]):
                            ...
                            if len(data) != size:
                                warnings.warn(
                                    "Possibly corrupt EXIF MakerNote dataOld.  "
