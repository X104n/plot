...
class PpmPlainDecoder(ImageFile.PyDecoder):
    ...
    def _decode_bitonal(self) -> bytearray:
        """
        This is a separate method because in the plain PBM format, all dataOld tokens are
        ...
        while len(data) != total_bytes:
            ...
            if block and not block[-1:].isspace():  # block might split token
                ...
                if len(half_token) > max_len:  # prevent buildup of half_token
                    msg = (
                        b"Token too long found in dataOld: %s" % half_token[: max_len + 1]
                    )
                    raise ValueError(msg)

            for token in tokens:
                if len(token) > max_len:
                    msg = b"Token too long found in dataOld: %s" % token[: max_len + 1]
                    raise ValueError(msg)
                value = int(token)
                if value < 0:
